...
// Macroestado de la compra (3 pestañas principales)
export type PurchaseMacroState = 'por_hacer' | 'en_ejecucion' | 'realizado';
...
export interface Purchase {
  ...
  modalidadCompra?: 'Convenio Marco' | 'Licitación Pública' | 'Licitación Privada' | 'Trato Directo' | 'Compra Ágil' | 'Fondo Fijo' | 'Otro' | string;
  units?: number;
  ...
  macroState?: PurchaseMacroState; // 'por_hacer' | 'en_ejecucion' | 'realizado'
...
/**
 * Determina el Macroestado efectivo de una compra basándose en sus microestados o estado explícito:
 * - 'por_hacer': compras planificadas/solicitadas que aún no tienen OC ni proceso en ejecución
 * - 'realizado': cuando la recepción es conforme Y la factura está pagada/cerrada (o marcado cerrado explícitamente)
 * - 'en_ejecucion': compras con OC emitida, despacho en curso, recepción pendiente, o recepción realizada con factura pendiente
 */
export function getPurchaseEffectiveMacroState(purchase: Purchase): PurchaseMacroState {
  if (purchase.macroState) {
    return purchase.macroState;
  }
...
export function getPurchaseAlerts(purchase: Purchase): PurchaseOperationalAlert[] {
  ...
  // 2. Recepción rechazada
  if (recStatus === 'rechazada') {
    alerts.push({
  ...
  // 3. Recepción con observaciones
  if (recStatus === 'con_observaciones') {
    alerts.push({
  ...
  // 4. Recepción Conforme hace tiempo pero sin factura
  if (recStatus === 'conforme' && invStatus === 'sin_factura' && macro !== 'realizado') {
    alerts.push({
  ...
  if ((invStatus === 'recibida' || invStatus === 'en_revision') && macro !== 'realizado') {
    alerts.push({
  ...
  // 6. OC emitida pero recepción pendiente
  if (purchase.purchaseOrderNumber && recStatus === 'pendiente' && macro === 'en_ejecucion') {
    alerts.push({
      id: `${purchase.id}-oc-sin-recepcion`,
      type: 'oc_sin_recepcion',
      title: 'OC Emitida — Esperando Despacho / Recepción',
      description: `Orden de Compra ${purchase.purchaseOrderNumber} activa. Esperando entrega por parte de ${purchase.supplier || 'proveedor'}.`,
