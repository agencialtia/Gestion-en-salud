...
export const DocumentosView: React.FC<DocumentosViewProps> = ({
  ...
  const baseDocsForProgram = useMemo(() => {
    return documents.filter((doc) => {
      ...
      // Program filter
      const pIds = Array.isArray(doc.programIds) ? doc.programIds : (doc.programId ? [doc.programId] : []);

      if (scopedProgramId) {
        if (pIds.length > 0 && !pIds.includes(scopedProgramId)) {
          return false;
        }
      } else if (selectedProgramFilter !== 'all') {
        if (selectedProgramFilter === 'transversal') {
          if (pIds.length > 0) return false;
        } else if (!pIds.includes(selectedProgramFilter)) {
          return false;
        }
      }
  ...
  const filteredDocs = useMemo(() => {
    return baseDocsForProgram.filter((doc) => {
      ...
      if (searchQuery.trim()) {
        ...
        const pIds = Array.isArray(doc.programIds) ? doc.programIds : (doc.programId ? [doc.programId] : []);
        const matchedPrograms = pIds.some((pId) => {
        ...
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-2.5 justify-between">
          ...
          <div className="relative flex-1 min-w-[220px]">
            ...
            <input
              ...
              placeholder="Buscar por título, categoría, versión, archivo..."
