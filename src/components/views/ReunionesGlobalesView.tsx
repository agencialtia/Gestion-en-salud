...
import { extractVideoMeetingLink } from '../../utils/googleCalendarSync';
    ...
    return activeMeetings.filter((m) => {
      ...
      if (searchQuery.trim()) {
        ...
        const inParticipants = (m.participants || []).some((p) => {
          ...
          return name ? String(name).toLowerCase().includes(query) : false;
        });
        ...
        const inAgreements = Array.isArray(m.agreements)
          ? m.agreements.some((a) => ((a && (a as any).description) || '').toLowerCase().includes(query))
          : typeof m.agreements === 'string'
          ? (m.agreements as string).toLowerCase().includes(query)
          : false;
                ...
                >
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      {(() => {
                        const video = extractVideoMeetingLink(meeting.meetingLink || meeting.location || meeting.summary || meeting.description);
                        ...
                        return (
                          <a
                            ...
                            className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-bold text-white transition-all shadow-2xs ${
                              video.platform === 'meet'
                  ...
                  {(meeting.description || meeting.summary) && (
                    <div className="text-xs text-slate-600 bg-slate-50/80 p-2.5 rounded-xl border border-slate-100 line-clamp-2">
