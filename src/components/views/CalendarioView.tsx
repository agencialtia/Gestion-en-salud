...
export const CalendarioView: React.FC<{
  ...
  const handleStartEdit = (m: Meeting, e?: React.MouseEvent) => {
    ...
    setEditFormData({
      ...
      participantsStr: (m.participants || []).map((p: any) => (typeof p === 'string' ? p : (p ? p.name : ''))).filter(Boolean).join(', '),
      status: (m.status as any) || 'programada',
    });
    setEditingMeeting(m);
  };
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    const updates: Partial<Meeting> = {
      ...
      participants: participantsList.map((name) => ({ name, role: '' })) as any,
