...
export const ProgramDetailView: React.FC<{
  ...
  const [taskFilter, setTaskFilter] = useState<'pendiente' | 'en_ejecucion' | 'completado'>('pendiente');
  const [meetingCommitmentFilter, setMeetingCommitmentFilter] = useState<'pendiente' | 'en_ejecucion' | 'completado'>('pendiente');
  const [emailStatusFilter, setEmailStatusFilter] = useState<'todos' | 'pendiente' | 'en_gestion' | 'respondido' | 'cerrado'>('todos');
  ...
  return (
    <div id="view-program-detail" className="space-y-6 animate-in fade-in duration-150 text-left">
      ...
      {activeTab === 'tareas' && (() => {
        const pendientesTasks = programTasks.filter((t) => t.status === 'por_hacer' || t.status === 'pendiente');
        const enEjecucionTasks = programTasks.filter((t) => t.status === 'en_ejecucion' || t.status === 'en_curso' || t.status === 'bloqueada');
        const completadasTasks = programTasks.filter((t) => t.status === 'terminada' || t.status === 'completada');
        ...
        return (
          <div className="space-y-4">
            ...
            {/* 3 Botones de Filtro: pendiente, en ejecucion, completado */}
            <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200 w-fit">
              ...
              <button
                ...
                onClick={() => setTaskFilter('en_ejecucion')}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  taskFilter === 'en_ejecucion'
              ...
              <button
                ...
                onClick={() => setTaskFilter('completado')}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              ...
              {displayedTasks.length > 0 ? (
                displayedTasks.map((t) => {
                  ...
                  return (
                    <div
                      key={t.id}
                      className="p-3.5 rounded-2xl border border-slate-200 bg-white hover:border-slate-300 flex items-center justify-between gap-3 text-xs shadow-2xs transition-all"
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <button
                          ...
                          className={`h-5 w-5 rounded border flex items-center justify-center transition-colors shrink-0 cursor-pointer ${
                            isDone
                        ...
                        <div className="space-y-0.5 min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 text-[11px] text-slate-500 flex-wrap">
                            <span>Resp: {t.responsible || 'Sin asignar'}</span>
                            <span>•</span>
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <TaskStatusChip 
                          status={t.status} 
                          onChange={(newStatus) => quickUpdateTaskStatus(t.id, newStatus)} 
                        />
                        <button
                          type="button"
                          onClick={() => onOpenEntity('task', t.id)}
                ...
                <div className="p-8 text-center bg-slate-50 border border-dashed border-slate-200 rounded-2xl space-y-2">
                  <p className="text-xs text-slate-500 font-medium">
                    No hay tareas en estado{' '}
                    <strong className="text-slate-700">
                      {taskFilter === 'pendiente' ? 'Pendiente' : taskFilter === 'en_ejecucion' ? 'En ejecución' : 'Completado'}
                    </strong>{' '}
                    para este programa.
                  </p>
                  <button
                  ...
                  >
                    <Plus className="h-3 w-3" /> Registrar nueva tarea
                  </button>
                </div>
              )}
            </div>
      ...
      {activeTab === 'reuniones' && (() => {
        ...
        const pendientesMeetings = programMeetings.filter((m) => normalizeMeetingStatus(m.status) === 'pendiente');
        const enEjecucionMeetings = programMeetings.filter((m) => normalizeMeetingStatus(m.status) === 'en_ejecucion');
        const completadasMeetings = programMeetings.filter((m) => normalizeMeetingStatus(m.status) === 'completado');
        ...
        return (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-bold text-slate-900">
                  Reuniones, Comités y Acuerdos ({programMeetings.length})
                </h3>
                <p className="text-xs text-slate-500">
                  Cadena rectora: Instancia → Acuerdo → Compromiso → Tarea sincronizada
                </p>
              </div>
              <button
            ...
            <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200 w-fit">
              <button
                ...
                onClick={() => setMeetingCommitmentFilter('pendiente')}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  meetingCommitmentFilter === 'pendiente'
                    ? 'bg-white text-slate-800 shadow-xs border border-slate-200/80'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
              ...
              <button
                ...
                onClick={() => setMeetingCommitmentFilter('en_ejecucion')}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  meetingCommitmentFilter === 'en_ejecucion'
                    ? 'bg-white text-blue-800 shadow-xs border border-blue-200/80'
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
              ...
              <button
                ...
                onClick={() => setMeetingCommitmentFilter('completado')}
                className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              ...
              >
                ...
                >
                  {completadasMeetings.length}
                </span>
              </button>
            </div>

            {programMeetings.length === 0 ? (
              <div className="p-8 text-center bg-slate-50 border border-dashed border-slate-200 rounded-2xl space-y-2">
                <Users className="w-8 h-8 text-slate-300 mx-auto" />
                <p className="text-xs text-slate-500 font-medium">
                  No hay reuniones o comités registrados para este programa.
                </p>
                <button
                  type="button"
                  onClick={() => onOpenQuickCreate('meeting')}
              ...
              <div className="p-8 text-center bg-slate-50 border border-dashed border-slate-200 rounded-2xl space-y-2">
                <Users className="w-8 h-8 text-slate-300 mx-auto" />
                <p className="text-xs text-slate-500 font-medium">
                  No hay reuniones en estado{' '}
                  <strong className="text-slate-700">
                    {meetingCommitmentFilter === 'pendiente' ? 'Pendiente' : meetingCommitmentFilter === 'en_ejecucion' ? 'En ejecución' : 'Completado'}
                  </strong>{' '}
                  para este programa.
                </p>
                <button
              ...
              <div className="space-y-4">
                {displayedMeetings.map((m) => {
                  ...
                  return (
                    <div key={m.id} className="p-5 rounded-2xl border border-slate-200 bg-white space-y-3.5 shadow-xs">
                        ...
                        <div className="space-y-1">
                          <div className="flex flex-wrap items-center gap-2">
                            <MeetingStatusChip
                              status={m.status}
                              onChange={(newStatus) => {
                                updateMeeting(m.id, { status: newStatus });
                                setMeetingCommitmentFilter(newStatus);
                              }}
                            />
                            {overdueComs > 0 && (
      ...
      {activeTab === 'correos' && (() => {
        ...
        const filteredEmails = programEmails.filter((e) => {
          ...
          if (emailStatusFilter !== 'todos' && canonical !== emailStatusFilter) {
            return false;
          }
          ...
          if (emailUrgencyFilter === 'vencidos') {
            if (!isEmailOverdue(e, todayStr)) return false;
          } else if (emailUrgencyFilter === 'hoy') {
            if (canonical === 'cerrado' || canonical === 'respondido' || e.deadline !== todayStr) return false;
          }
        ...
        // Compute KPI counts
        const totalCount = programEmails.length;
        const pendingCount = programEmails.filter((e) => normalizeCommunicationStatus(e.status) === 'pendiente').length;
        const inProgressCount = programEmails.filter((e) => normalizeCommunicationStatus(e.status) === 'en_gestion').length;
        const overdueCount = programEmails.filter((e) => isEmailOverdue(e, todayStr)).length;
        const dueTodayCount = programEmails.filter((e) => {
          const c = normalizeCommunicationStatus(e.status);
          return (c === 'pendiente' || c === 'en_gestion') && e.deadline === todayStr;
        }).length;
        const resolvedCount = programEmails.filter((e) => {
          const c = normalizeCommunicationStatus(e.status);
          return c === 'respondido' || c === 'cerrado';
        }).length;
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold text-slate-900">
                    Control Operativo de Correos, Oficios y Requerimientos
                  </h3>
                  <span className="px-2 py-0.5 rounded-full bg-slate-100 text-[11px] font-bold text-slate-700">
                    {totalCount} total
                  </span>
                </div>
                <p className="text-xs text-slate-500 mt-0.5">
                  Seguimiento de plazos, acciones requeridas, tareas operativas y respuestas oficiales
                </p>
              </div>

              <button
                onClick={() => onOpenQuickCreate('email')}
                className="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 active:scale-95 text-white rounded-xl text-xs font-bold shadow-xs transition-all shrink-0"
              >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5">
              <button
                onClick={() => { setEmailStatusFilter('pendiente'); setEmailUrgencyFilter('todos'); }}
                className={`p-3 rounded-xl border text-left transition-all ${
                  emailStatusFilter === 'pendiente' && emailUrgencyFilter === 'todos'
                    ? 'border-amber-400 bg-amber-50/70 ring-2 ring-amber-200'
                    : 'border-slate-200 bg-white hover:border-amber-200'
                }`}
              >
                <div className="text-[11px] font-semibold text-amber-700">Pendientes</div>
                <div className="text-lg font-bold text-slate-900 mt-0.5">{pendingCount}</div>
                <div className="text-[10px] text-slate-500">Por iniciar gestión</div>
              </button>

              <button
                onClick={() => { setEmailStatusFilter('en_gestion'); setEmailUrgencyFilter('todos'); }}
                className={`p-3 rounded-xl border text-left transition-all ${
                  emailStatusFilter === 'en_gestion' && emailUrgencyFilter === 'todos'
                    ? 'border-blue-400 bg-blue-50/70 ring-2 ring-blue-200'
                    : 'border-slate-200 bg-white hover:border-blue-200'
                }`}
              >
                <div className="text-[11px] font-semibold text-blue-700">En Gestión</div>
                <div className="text-lg font-bold text-slate-900 mt-0.5">{inProgressCount}</div>
                <div className="text-[10px] text-slate-500">En desarrollo activo</div>
              </button>

              <button
                onClick={() => { setEmailUrgencyFilter('vencidos'); setEmailStatusFilter('todos'); }}
                className={`p-3 rounded-xl border text-left transition-all ${
                  emailUrgencyFilter === 'vencidos'
              ...
              >
                <div className="text-[11px] font-semibold text-rose-700 flex items-center justify-between">
                  <span>Vencidos</span>
                  {overdueCount > 0 && <span className="h-2 w-2 rounded-full bg-rose-500 animate-pulse" />}
                </div>
                <div className="text-lg font-bold text-rose-700 mt-0.5">{overdueCount}</div>
                <div className="text-[10px] text-slate-500">Plazo superado</div>
              </button>

              <button
                onClick={() => { setEmailUrgencyFilter('hoy'); setEmailStatusFilter('todos'); }}
                className={`p-3 rounded-xl border text-left transition-all ${
                  emailUrgencyFilter === 'hoy'
                    ? 'border-orange-400 bg-orange-50/70 ring-2 ring-orange-200'
                    : 'border-slate-200 bg-white hover:border-orange-200'
                }`}
              >
                <div className="text-[11px] font-semibold text-orange-700">Vencen Hoy</div>
                <div className="text-lg font-bold text-slate-900 mt-0.5">{dueTodayCount}</div>
                <div className="text-[10px] text-slate-500">Plazo al límite</div>
              </button>

              <button
                onClick={() => { setEmailStatusFilter('cerrado'); setEmailUrgencyFilter('todos'); }}
                className={`p-3 rounded-xl border text-left transition-all ${
                  emailStatusFilter === 'cerrado' && emailUrgencyFilter === 'todos'
                    ? 'border-emerald-400 bg-emerald-50/70 ring-2 ring-emerald-200'
                    : 'border-slate-200 bg-white hover:border-emerald-200'
                }`}
              >
                <div className="text-[11px] font-semibold text-emerald-700">Respondidos / Cerrados</div>
                <div className="text-lg font-bold text-slate-900 mt-0.5">{resolvedCount}</div>
                <div className="text-[10px] text-slate-500">Completados</div>
              </button>
            </div>

            {/* Filters and Search Bar */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 bg-white p-3 rounded-xl border border-slate-200">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-xs font-bold text-slate-500 mr-1">Estado:</span>
                {(['todos', 'pendiente', 'en_gestion', 'respondido', 'cerrado'] as const).map((st) => (
                  <button
                    key={st}
                    onClick={() => { setEmailStatusFilter(st); setEmailUrgencyFilter('todos'); }}
                    className={`px-2.5 py-1 rounded-lg text-xs font-semibold transition-all ${
                      emailStatusFilter === st && emailUrgencyFilter === 'todos'
                        ? 'bg-slate-900 text-white shadow-xs'
                        : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                    }`}
                  >
                    {st === 'todos' ? 'Todos' : st === 'en_gestion' ? 'En gestión' : st.charAt(0).toUpperCase() + st.slice(1)}
                  </button>
                ))}
              </div>
            ...
            {filteredEmails.length === 0 ? (
              <div className="p-8 rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center space-y-2">
                ...
                <button
                  onClick={() => onOpenQuickCreate('email')}
                  className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 transition-colors"
                >
              ...
              <div className="space-y-2.5">
                {filteredEmails.map((e) => {
                  ...
                  return (
                    <div
                      key={e.id}
                      className="p-4 rounded-2xl border border-slate-200 bg-white hover:border-indigo-300 transition-all shadow-xs space-y-3"
                    >
                      {/* Top Row: Type Badge + Title, Urgency Selector + Status Chip + Actions */}
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        ...
                        {/* Right side controls: Urgency selector (dropdown like tasks), Status chip, View, Delete */}
                        <div className="flex items-center gap-1.5 shrink-0 self-start">
                          ...
                          <CommunicationStatusChip
                            status={e.status}
                            onChange={(newStatus) => updateEmail(e.id, { status: newStatus })}
                          />
