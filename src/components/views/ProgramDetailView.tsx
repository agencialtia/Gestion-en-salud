...
import { ProgramId, TaskStatus, PurchaseStatus, PriorityLevel, KnowledgeCategory, QuestionStatus, FinancialPeriod, BudgetComponent, ProgramBudget2025Note, HRRecord } from '../../types';
import {
  TrendingUp,
  DollarSign,
  ShoppingBag,
...
export const ProgramDetailView: React.FC<{
  ...
  return (
    <div id="view-program-detail" className="space-y-6 animate-in fade-in duration-150 text-left">
      ...
      {activeTab === 'compras' && (() => {
        const porHacerPurchases = programPurchases.filter((p) => p.status === 'pendiente' || p.status === 'solicitado');
        const enEjecucionPurchases = programPurchases.filter((p) => p.status === 'en_compra' || p.status === 'problema');
        const realizadosPurchases = programPurchases.filter((p) => p.status === 'recepcionado' || p.status === 'cerrado');
        ...
        return (
          <div className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              {/* Botón Nueva Compra solo visible en Por hacer cuando hay 1 o más compras */}
              {purchaseFilter === 'por_hacer' && porHacerPurchases.length > 0 && (
                <button
                  type="button"
            ...
            {currentFilteredPurchases.length === 0 ? (
              <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200 space-y-1.5">
                ...
                <p className="text-[11px] text-slate-500 max-w-md mx-auto">
                  ...
                  {purchaseFilter === 'realizado' && 'Las compras recibidas conforme y cerradas aparecerán en esta sección.'}
                </p>
                {purchaseFilter === 'por_hacer' && (
                  <div className="pt-2">
                    <button
                  ...
                  {currentFilteredPurchases.map((p) => (
                    ...
                    >
                      ...
                      <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-2.5 rounded-xl">
                        ...
                        <div>
                          <span className="text-slate-400 block text-[10px]">Fecha Requerida:</span>
                          <span className="font-medium text-slate-700">{p.requiredDate}</span>
                        </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    <thead>
                      <tr className="border-b border-slate-200 bg-slate-100/60 font-semibold text-slate-600">
                        ...
                        <th className="p-3.5">Precio Total (c/IVA)</th>
                        <th className="p-3.5">Proveedor Elegido</th>
                        <th className="p-3.5">Modalidad / OC</th>
                        <th className="p-3.5">Link Ref.</th>
                      ...
                      {currentFilteredPurchases.map((p) => (
                        <tr key={p.id} className="hover:bg-slate-50">
                          ...
                          <td className="p-3.5 text-slate-600">
                            ...
                            {p.purchaseOrderNumber && (
                              <div className="text-[11px] font-mono text-indigo-600">{p.purchaseOrderNumber}</div>
                            )}
                          </td>
                          <td className="p-3.5">
                            {p.referenceLink && p.referenceLink.trim().length > 0 ? (
