...
export const ProgramDetailView: React.FC<{
  ...
  const [purchaseFilter, setPurchaseFilter] = useState<'por_hacer' | 'en_ejecucion' | 'realizado'>('por_hacer');
  ...
  return (
    <div id="view-program-detail" className="space-y-6 animate-in fade-in duration-150 text-left">
      ...
      {activeTab === 'compras' && (() => {
        const porHacerPurchases = programPurchases.filter((p) => getPurchaseEffectiveMacroState(p) === 'por_hacer');
        const enEjecucionPurchases = programPurchases.filter((p) => getPurchaseEffectiveMacroState(p) === 'en_ejecucion');
        const realizadosPurchases = programPurchases.filter((p) => getPurchaseEffectiveMacroState(p) === 'realizado');

        const porHacerTotal = porHacerPurchases.reduce((acc, p) => acc + ((p.totalPriceWithTax ?? p.estimatedAmount) || 0), 0);
        const enEjecucionTotal = enEjecucionPurchases.reduce((acc, p) => acc + ((p.totalPriceWithTax ?? p.estimatedAmount) || 0), 0);
        const realizadosTotal = realizadosPurchases.reduce((acc, p) => acc + ((p.totalPriceWithTax ?? p.estimatedAmount) || 0), 0);

        const currentFilteredPurchases =
          purchaseFilter === 'por_hacer'
            ? porHacerPurchases
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5 p-1 bg-slate-100 rounded-xl border border-slate-200">
                <button
                  type="button"
                  id="tab-compras-por-hacer"
                  onClick={() => setPurchaseFilter('por_hacer')}
                ...
                <button
                  type="button"
                  id="tab-compras-realizado"
                  onClick={() => setPurchaseFilter('realizado')}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    purchaseFilter === 'realizado'
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <div className="flex items-center gap-2 text-slate-600">
                ...
                <span>
                  {purchaseFilter === 'por_hacer' && 'Compras planificadas, presupuestadas o en formulación antes de iniciar gestión.'}
                  {purchaseFilter === 'en_ejecucion' && 'Compras con gestión iniciada: OC emitida, esperando despacho, recepción o con factura en trámite.'}
                  {purchaseFilter === 'realizado' && 'Compras con recepción conforme y proceso administrativo de facturación cerrado.'}
                </span>
              </div>
              <div className="font-mono font-bold text-slate-700 text-[11px] shrink-0">
                Total estimado / ejecutado: ${
                  (purchaseFilter === 'por_hacer' ? porHacerTotal : purchaseFilter === 'en_ejecucion' ? enEjecucionTotal : realizadosTotal).toLocaleString('es-CL')
                }
              </div>
            </div>
            ...
            {currentFilteredPurchases.length === 0 ? (
              <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200 space-y-1.5">
                <p className="text-xs font-bold text-slate-700">
                  {purchaseFilter === 'por_hacer' && 'No hay compras pendientes por gestionar'}
                  {purchaseFilter === 'en_ejecucion' && 'No hay compras en ejecución actualmente'}
                  {purchaseFilter === 'realizado' && 'No hay compras realizadas / recepcionadas aún'}
                </p>
                <p className="text-[11px] text-slate-500 max-w-md mx-auto">
                  {purchaseFilter === 'por_hacer' && 'Todas las compras han avanzado a ejecución o se encuentran finalizadas.'}
                  {purchaseFilter === 'en_ejecucion' && 'No hay órdenes de compra activas en gestión o en proceso con proveedores.'}
                  {purchaseFilter === 'realizado' && 'Las compras recibidas conforme y con factura cerrada aparecerán en esta sección.'}
                </p>
                <div className="pt-2">
                  <button
                    type="button"
                  ...
                  {currentFilteredPurchases.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Operative Badges: Reception & Invoice */}
                        ...
                        <div className="grid grid-cols-2 gap-2 text-[11px] bg-slate-50 p-2.5 rounded-xl">
                          ...
                          <div>
                            ...
                            <span className="font-medium text-slate-700">{p.requiredDate || p.orderAcceptedDate || '—'}</span>
                          </div>
                            ...
                            <td className="p-3.5 text-slate-600">
                              ...
                              {(p.requiredDate || p.orderAcceptedDate) && (
                                <div className="text-[11px] text-slate-500 mt-0.5">
                                  ...
                                  <span className="font-semibold text-slate-700">{p.requiredDate || p.orderAcceptedDate}</span>
                                </div>
                              )}
                            </td>
                            <td className="p-3.5">
