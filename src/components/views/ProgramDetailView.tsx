...
import {
  getProgramCutConfig,
  evaluateReliquidation,
  ProgramCutConfig,
  DEFAULT_PROGRAM_CUT_CONFIGS,
} from '../../utils/reliquidationUtils';
...
export const ProgramDetailView: React.FC<{
  ...
  const [selectedCut, setSelectedCut] = useState<'corte1' | 'corte2' | 'corte3'>('corte1');

  // Cut and Reliquidation Config State
  const [customCutConfigs, setCustomCutConfigs] = useState<Record<string, Record<'corte1' | 'corte2' | 'corte3', ProgramCutConfig>>>(() => {
    try {
      const saved = localStorage.getItem('qs_program_cut_configs');
      return saved ? JSON.parse(saved) : DEFAULT_PROGRAM_CUT_CONFIGS;
    } catch {
      return DEFAULT_PROGRAM_CUT_CONFIGS;
    }
  });
  ...
  const handleSaveCutConfig = (updatedConfig: ProgramCutConfig) => {
    setCustomCutConfigs((prev) => {
      const progConfigs = prev[updatedConfig.programId] || DEFAULT_PROGRAM_CUT_CONFIGS[updatedConfig.programId] || ({} as any);
      const next = {
        ...prev,
        [updatedConfig.programId]: {
          ...progConfigs,
          [updatedConfig.cutKey]: updatedConfig,
        },
      };
      try {
        localStorage.setItem('qs_program_cut_configs', JSON.stringify(next));
      } catch (err) {
        console.error(err);
      }
      return next;
  ...
  // Filter states
  const [purchaseFilter, setPurchaseFilter] = useState<'todos' | 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido' | 'por_hacer' | 'realizado'>('todos');
  ...
  const [dragOverPurchaseCard, setDragOverPurchaseCard] = useState<'todos' | 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido' | null>(null);
  ...
  const [dragOverTaskCard, setDragOverTaskCard] = useState<'todos' | 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido' | null>(null);
  ...
  const [dragOverMeetingCard, setDragOverMeetingCard] = useState<'todos' | 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido' | null>(null);
  ...
  const [dragOverCard, setDragOverCard] = useState<'todos' | 'pendiente' | 'en_gestion' | 'cerrado' | 'vence_hoy' | 'vencido' | null>(null);
  ...
  const [hrForm, setHrForm] = useState<{
    ...
    workdayHours: number;
    programHours: number;
    contractType: 'Contrata' | 'Planta' | 'Honorarios';
    status: 'activo' | 'en_proceso_seleccion';
    functions: string;
    notes: string;
  }>({
    name: '',
  ...
  const handleOpenEditHR = (h: HRRecord) => {
    ...
    setHrForm({
      ...
      contractType: ((h.contractType === 'Honorarios' || h.contractType === 'Planta' ? h.contractType : 'Contrata') as 'Contrata' | 'Honorarios' | 'Planta'),
      status: (h.status === 'en_proceso_seleccion' ? 'en_proceso_seleccion' : 'activo'),
  ...
  const currentProgram =
    (programs && programs.length > 0 ? programs.find((p) => p.id === selectedProgramId) || programs[0] : null) ||
    HEALTH_PROGRAMS[0];

  const summary = currentProgram && programSummaries ? programSummaries[currentProgram.id] : undefined;

  // Scoped Data
  const programTasks = currentProgram ? (tasks || []).filter((t) => !t.archived && t.programId === currentProgram.id) : [];
  const programIndicators = currentProgram ? (indicators || []).filter((i) => !i.archived && i.programId === currentProgram.id) : [];
  const programFinancials = currentProgram ? (financialPeriods || []).filter((f) => f.programId === currentProgram.id) : [];
  const progComponents = currentProgram ? (budgetComponents || []).filter((c) => c.programId === currentProgram.id) : [];
  ...
  const programContacts = currentProgram
    ? (contacts || []).filter((c) => {
        if (c.archived) return false;
        const pIds = Array.isArray(c.programIds) ? c.programIds : (c.programId ? [c.programId] : []);
        return pIds.length === 0 || pIds.includes(currentProgram.id);
      })
    : [];
  const programDocuments = currentProgram
    ? (documents || []).filter((d) => {
        if (d.archived) return false;
        const pIds = Array.isArray(d.programIds) ? d.programIds : (d.programId ? [d.programId] : []);
        return pIds.length === 0 || pIds.includes(currentProgram.id);
      })
    : [];
  ...
  const resolvePreviousYearBudget = () => {
    ...
    const stored = (budget2025Notes && budget2025Notes[key]) || (previousYear === 2025 && budget2025Notes ? budget2025Notes[currentProgram.id] : undefined);
  ...
  const programPurchases = currentProgram ? (purchases || []).filter((p) => !p.archived && p.programId === currentProgram.id) : [];
  const programMeetings = currentProgram ? (meetings || []).filter((m) => !m.archived && m.programId === currentProgram.id) : [];
  const programEmails = currentProgram ? (emails || []).filter((e) => !e.archived && e.programId === currentProgram.id) : [];
  const programQuestions = currentProgram ? (questions || []).filter((q) => !q.archived && q.programId === currentProgram.id) : [];
  const programKnowledge = currentProgram
    ? (knowledge || []).filter((k) => {
        if (k.archived) return false;
        const pIds = Array.isArray(k.programIds) ? k.programIds : (k.programId ? [k.programId] : []);
        return pIds.length === 0 || pIds.includes(currentProgram.id) || pIds.includes('all') || k.programId === currentProgram.id;
      })
    : [];
  const programHR = currentProgram ? (hrRecords || []).filter((h) => !h.archived && h.programId === currentProgram.id) : [];
  ...
  const tabs = [
    ...
    { id: 'contactos', label: `Contactos (${programContacts.length})`, icon: Users },
    { id: 'documentos', label: `Documentos (${programDocuments.length})`, icon: FileText },
    ...
    { id: 'conocimiento', label: `Conocimiento (${programKnowledge.length})`, icon: BookOpen },
  ];
  ...
  return (
    <div id="view-program-detail" className="space-y-6 animate-in fade-in duration-150 text-left">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
        ...
        {/* Mobile View: Dropdown Selector for mobile screens */}
        <div className="md:hidden pt-2.5 border-t border-slate-100">
          <label htmlFor="mobile-program-tab-select" className="block text-[11px] font-bold text-slate-500 mb-1.5 uppercase tracking-wider">
            Sección del Programa:
          </label>
          <div className="relative">
            <select
              id="mobile-program-tab-select"
              value={activeTab}
              onChange={(e) => setActiveTab(e.target.value as any)}
              className="w-full appearance-none bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-900 text-xs font-bold rounded-xl px-3.5 py-2.5 pr-9 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 cursor-pointer shadow-xs transition-all"
            >
              {tabs.map((tab) => (
        ...
        {/* Tablet & Desktop View: Buttons List (>= md: 768px) */}
        <div className="hidden md:flex flex-wrap items-center gap-1.5 sm:gap-2 pt-2.5 border-t border-slate-100">
          {tabs.map((tab) => {
            const Icon = tab.icon;
              ...
              <div className="flex items-center gap-4 text-xs">
                <div>
                  ...
                  <strong className="text-slate-800">
                    {typeof summary.indicatorsCompliance === 'number' && !isNaN(summary.indicatorsCompliance)
                      ? summary.indicatorsCompliance.toFixed(0)
                      : '0'}%
                  </strong>
                </div>
                ...
                <div>
                  ...
                  <strong className={(summary.financialExecutionRate || 0) < 45 ? 'text-rose-600' : 'text-slate-800'}>
                    {typeof summary.financialExecutionRate === 'number' && !isNaN(summary.financialExecutionRate)
                ...
                <div>
                  ...
                  <strong className={(summary.overdueTasksCount || 0) > 0 ? 'text-rose-600' : 'text-slate-800'}>
                  ...
                  <div key={ind.id} className="space-y-1">
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-bold text-slate-900">
                        {ind.currentResult ?? (ind as any).currentValue ?? 0} / {ind.periodTarget ?? (ind as any).targetValue ?? 100}
                      </span>
                    </div>
                    <ProgressBar
                      value={
                        (ind.periodTarget || (ind as any).targetValue || 0) > 0
                          ? (((ind.currentResult ?? (ind as any).currentValue ?? 0) / (ind.periodTarget || (ind as any).targetValue || 1)) * 100)
                          : 0
                      }
                      size="sm"
                    />
        ...
        <div className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              {/* Botones de 1° corte, 2° corte y 3° corte */}
              <div className="inline-flex items-center p-1 bg-slate-100/90 rounded-xl border border-slate-200 shadow-inner">
                ...
                {programIndicators.some((i) => i.corte3 || i.corteSeleccionado === '3° corte') && (
                  <button
                    type="button"
                    onClick={() => setSelectedCut('corte3')}
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all duration-150 cursor-pointer ${
                      selectedCut === 'corte3'
                        ? 'bg-white text-indigo-700 shadow-sm ring-1 ring-black/5'
                        : 'text-slate-600 hover:text-slate-900 hover:bg-white/50'
          ...
          {/* Cuadro Resumen Global de Indicadores (Color Indigo Royal matching Image 4 con Tabla de Cortes y Reliquidación SSMN) */}
          {(() => {
            const cutConfig = getProgramCutConfig(currentProgram.id, selectedCut, customCutConfigs);
            ...
            const items = programIndicators.map((ind) => {
              const cutTargetRaw = selectedCut === 'corte1'
                ? (ind.corte1?.target ?? ind.periodTarget ?? ind.annualTarget)
                : selectedCut === 'corte2'
                ? (ind.corte2?.target ?? ind.annualTarget)
                : (ind.corte3?.target ?? ind.annualTarget);

              const cutResultRaw = selectedCut === 'corte1'
                ? (ind.corte1?.result ?? (ind.corteSeleccionado === '1° corte' ? ind.currentResult : ind.currentResult))
                : selectedCut === 'corte2'
                ? (ind.corte2?.result ?? (ind.corteSeleccionado === '2° corte' ? ind.currentResult : (ind.corte2 ? ind.corte2.result : ind.currentResult)))
                : (ind.corte3?.result ?? (ind.corteSeleccionado === '3° corte' ? ind.currentResult : (ind.corte3 ? ind.corte3.result : ind.currentResult)));

              const cutTarget = typeof cutTargetRaw === 'number' && !isNaN(cutTargetRaw)
                ? cutTargetRaw
                : (parseFloat(cutTargetRaw as any) || (ind.annualTarget ? Number(ind.annualTarget) : 0) || 0);

              const cutResult = typeof cutResultRaw === 'number' && !isNaN(cutResultRaw)
                ? cutResultRaw
                : (parseFloat(cutResultRaw as any) || (ind.currentResult ? Number(ind.currentResult) : 0) || 0);

              let percent = 0;
              if (ind.direction === 'higher_is_better') {
                percent = cutTarget > 0 ? (cutResult / cutTarget) * 100 : (cutResult > 0 ? 100 : 0);
              } else {
                percent = cutResult > 0 ? (cutTarget / cutResult) * 100 : 100;
              }

              let weight = ind.pesoRelativo;
              if (!hasExplicitWeights || weight === undefined || isNaN(weight) || weight <= 0) {
              ...
              totalWeight += weight;
              const contribution = Number.isFinite((percent * weight) / 100) ? (percent * weight) / 100 : 0;
              const weightedFinalResult = Number.isFinite((cutResult * weight) / 100) ? (cutResult * weight) / 100 : 0;
              const weightedTarget = Number.isFinite((cutTarget * weight) / 100) ? (cutTarget * weight) / 100 : 0;
            ...
            // Sum of the Cumplimiento column (Resultado final de cada indicador según peso relativo)
            const sumWeightedCompliance = Number(
              items.reduce((acc, item) => acc + (Number.isFinite(item.weightedFinalResult) ? item.weightedFinalResult : 0), 0).toFixed(2)
            );
            const sumTargetWeighted = Number(
              items.reduce((acc, item) => acc + (Number.isFinite(item.weightedTarget) ? item.weightedTarget : 0), 0).toFixed(2)
            );
            const relativeAchievementRatio = sumTargetWeighted > 0 ? (sumWeightedCompliance / sumTargetWeighted) * 100 : sumWeightedCompliance;

            // Reliquidation & Cutoff Evaluation per specific health program
            const reliquidation = evaluateReliquidation(sumWeightedCompliance, cutConfig, currentProgram.annualBudget);
            const meetsGoal = reliquidation.meetsGoal;
            const formattedCompliance = Number.isFinite(sumWeightedCompliance) ? sumWeightedCompliance : 0;
            const cutLabel = selectedCut === 'corte1' ? '1° Corte' : selectedCut === 'corte2' ? '2° Corte' : '3° Corte';

            // Find established cutoff date for the selected cut
            const activeCutDate = cutConfig.cutoffDate || programIndicators.find((ind) => {
              if (selectedCut === 'corte1') return ind.corte1?.date || (ind.corteSeleccionado === '1° corte' ? ind.cutoffDate : null);
              if (selectedCut === 'corte2') return ind.corte2?.date || (ind.corteSeleccionado === '2° corte' ? ind.cutoffDate : null);
              return ind.corte3?.date || (ind.corteSeleccionado === '3° corte' ? ind.cutoffDate : null);
            })?.cutoffDate || programIndicators[0]?.cutoffDate || (selectedCut === 'corte1' ? '2026-07-31' : selectedCut === 'corte2' ? '2026-11-30' : '2026-12-31');

            const formattedCutDate = (() => {
              if (!activeCutDate) return '31/07/2026';
              if (activeCutDate.includes('-')) {
                const parts = activeCutDate.split('-');
                if (parts.length === 3) return `${parts[2]}/${parts[1]}/${parts[0]}`;
              }
              return activeCutDate;
            })();

            return (
              <div className="rounded-2xl bg-indigo-600 text-white p-3.5 sm:p-5 shadow-md border border-indigo-500/60 space-y-4">
                {/* Header of Global Summary */}
                <div className="flex flex-wrap items-center justify-between gap-2.5">
                  <div className="flex items-center gap-2.5">
                    <div className="p-2 rounded-xl bg-white/15 backdrop-blur-xs border border-white/20 text-white shadow-xs">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-white/20 text-white border border-white/20 shadow-2xs">
                        {cutLabel}
                      </span>
                      <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-indigo-950/40 text-indigo-100 border border-indigo-300/30 flex items-center gap-1">
                        <Calendar className="h-3 w-3" />
                        Fecha de Corte: {formattedCutDate}
                      </span>
                      <span className="text-[10px] font-bold px-2.5 py-0.5 rounded-full bg-indigo-900/60 text-indigo-200 border border-indigo-300/20">
                        {cutConfig.quotaEvaluated}
                      </span>
                    </div>
                  </div>

                  {/* Dynamic Status: CUMPLE / NO CUMPLE (driven by Cut Reliquidation Table) */}
                  <div className="flex items-center gap-2">
                    {meetsGoal ? (
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-emerald-500 text-white text-[11px] sm:text-xs font-black tracking-wide shadow-xs uppercase border border-emerald-400">
                        <CheckCircle2 className="h-3.5 w-3.5 stroke-[2.5]" />
                        <span>Cumple (Sin Reliquidación)</span>
                      </div>
                    ) : (
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-xl bg-rose-500 text-white text-[11px] sm:text-xs font-black tracking-wide shadow-xs uppercase border border-rose-400">
                        <AlertTriangle className="h-3.5 w-3.5 stroke-[2.5]" />
                        <span>No Cumple (Reliquidación {reliquidation.discountPercentage}%)</span>
                      </div>
                    )}
                  </div>
                </div>

                {/* % Cumplimiento Global Metric Card */}
                <div className="bg-indigo-700/80 rounded-xl p-3.5 border border-indigo-400/30 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] sm:text-[11px] font-bold text-indigo-200 uppercase tracking-wider">
                      % Cumplimiento Global ({cutLabel})
                    </span>
                    <span className="text-[10px] text-indigo-200 font-semibold">
                      Suma de cumplimiento de indicadores
                    </span>
                  </div>
                    ...
                    <div className="flex items-baseline gap-2">
                      ...
                      {sumTargetWeighted > 0 && (
                        <span className="text-xs text-indigo-200 font-medium">
                          de {Number(sumTargetWeighted.toFixed(2))}% ponderado máx.
                        </span>
                      )}
                    </div>
                    <div className="text-right">
                      <span className="text-xs font-bold text-indigo-100 block">
                        Meta para 0% reliquidación: <strong className="text-white">≥{cutConfig.targetCompliance}%</strong>
                      </span>
                      {reliquidation.gapToCompliance > 0 ? (
                        <span className="text-[11px] text-rose-200 font-semibold">
                          Brecha: Faltan {reliquidation.gapToCompliance}% para evitar reliquidación
                        </span>
                      ) : (
                        <span className="text-[11px] text-emerald-200 font-semibold">
                          ✓ Cumple el umbral del corte establecido
                        </span>
                      )}
                    </div>
                  </div>
                  <div className="w-full h-2.5 rounded-full bg-indigo-950/60 overflow-hidden relative">
                    <div
                      className={`h-full rounded-full transition-all duration-300 ${
                        meetsGoal ? 'bg-emerald-400' : 'bg-rose-400'
                      }`}
                      style={{ width: `${Math.min(100, Math.max(0, (formattedCompliance / Math.max(100, cutConfig.targetCompliance)) * 100))}%` }}
                    />
                  </div>
                </div>

                {/* Financial Reliquidation Assessment Banner */}
                <div
                  className={`rounded-xl p-3 text-xs border flex items-start justify-between gap-3 ${
                    meetsGoal
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      <div className="font-bold text-white text-xs sm:text-sm">
                        {meetsGoal
                          ? 'Sin Descuento de Recursos por Reliquidación SSMN'
                          : `Riesgo de Reliquidación: ${reliquidation.discountPercentage}% de Descuento en la ${cutConfig.quotaEvaluated}`}
                      </div>
                      <p className="mt-0.5 leading-relaxed text-indigo-100">
                        {meetsGoal ? (
                          <>
                            Al alcanzar el <strong>{formattedCompliance}%</strong> de cumplimiento global, el programa se sitúa en el tramo óptimo (≥{cutConfig.targetCompliance}%). El Servicio de Salud transferirá el 100% de la <strong>{cutConfig.quotaEvaluated}</strong> sin descuentos.
                          </>
                        ) : (
                          <>
                            Con un <strong>{formattedCompliance}%</strong> de cumplimiento global, el programa cae en el tramo <strong>'{reliquidation.activeTranche.label}'</strong>. El Servicio de Salud Metropolitano Norte (SSMN) descontará el <strong>{reliquidation.discountPercentage}%</strong> de los recursos correspondientes a la <strong>{cutConfig.quotaEvaluated}</strong>.
                            {reliquidation.estimatedPenaltyAmount ? (
                              <span className="block mt-1 font-semibold text-rose-200">
                                Impacto financiero estimado en riesgo: ${reliquidation.estimatedPenaltyAmount.toLocaleString('es-CL')} CLP (sobre una cuota total de ${reliquidation.estimatedQuotaAmount?.toLocaleString('es-CL')} CLP).
                              </span>
                            ) : null}
                          </>
                        )}
                ...
                {/* Tabla de Cortes y Descuento de Recursos (SSMN) Integrada */}
                <div className="bg-indigo-900/60 rounded-xl p-3 sm:p-4 border border-indigo-400/25 space-y-2.5">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      <ShieldAlert className="h-4 w-4 text-indigo-200" />
                      <span className="text-xs font-bold text-white uppercase tracking-wider">
                        Tabla de Cortes y Descuento de Recursos SSMN — {cutLabel}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setShowFullCutTable(!showFullCutTable)}
                        className="text-[11px] font-semibold text-indigo-200 hover:text-white underline cursor-pointer"
                      >
                        {showFullCutTable ? 'Ocultar tabla' : 'Ver tabla completa'}
                      </button>
                      <button
                        type="button"
                        onClick={() => setIsCutTableModalOpen(true)}
                        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-white text-[11px] font-bold border border-white/20 transition-all cursor-pointer"
                      >
                  ...
                  {showFullCutTable && (
                    <div className="border border-indigo-400/30 rounded-xl overflow-hidden shadow-inner">
                      <table className="w-full text-xs text-left border-collapse">
                        <thead className="bg-indigo-950/70 border-b border-indigo-400/30 text-indigo-200 font-bold">
                          <tr>
                            <th className="p-2.5 sm:p-3">Porcentaje de Cumplimiento</th>
                            <th className="p-2.5 sm:p-3 text-center">
                              Descuento de Recursos ({cutConfig.quotaEvaluated})
                            </th>
                            <th className="p-2.5 sm:p-3">Condición / Estado</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-indigo-400/20">
                          {cutConfig.tranches.map((tranche, idx) => {
                            const isActive = idx === reliquidation.activeTrancheIndex;
                            const isCompliant = tranche.discountPercentage === 0;
                            return (
                              <tr
                                key={tranche.id || idx}
                                className={`transition-all duration-200 ${
                                  isActive
                                    ? isCompliant
                                      ? 'bg-emerald-600/35 text-white font-bold border-y-2 border-emerald-400 ring-2 ring-inset ring-emerald-400/90 shadow-md'
                                      : 'bg-amber-500/30 text-amber-50 font-bold border-y-2 border-amber-400 ring-2 ring-inset ring-amber-400/90 shadow-md'
                              ...
                              >
                                <td className="p-2.5 sm:p-3">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    {isActive && (
                                      <span
                                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider shadow-sm ${
                                          isCompliant
                                ...
                                <td className="p-2.5 sm:p-3 text-center">
                                  <span
                                    className={`inline-block px-2.5 py-1 rounded-lg text-xs font-mono font-black shadow-2xs ${
                                      isActive
                                        ? isCompliant
                                          ? 'bg-emerald-400 text-emerald-950 ring-1 ring-emerald-300'
                                          : 'bg-amber-400 text-slate-950 ring-1 ring-amber-300'
                                        : tranche.discountPercentage === 0
                                        ? 'bg-emerald-500/25 text-emerald-200 border border-emerald-400/40'
                                        : 'bg-rose-500/25 text-rose-200 border border-rose-400/40'
                                    }`}
                                  >
                                    {tranche.discountPercentage}%
                                  </span>
                                </td>
                                <td className="p-2.5 sm:p-3 text-xs">
                                  {tranche.discountPercentage === 0 ? (
                                    <span
                                      className={`font-bold flex items-center gap-1 ${
                                        isActive ? 'text-emerald-200' : 'text-emerald-300 font-semibold'
                                      }`}
                                    >
                                      ✓ Sin reliquidación (Cumple meta)
                                    </span>
                                  ) : (
                                    <span
                                      className={`font-bold flex items-center gap-1 ${
                                        isActive ? 'text-amber-200' : 'text-rose-300 font-medium'
                                      }`}
                                    >
                                      ⚠️ Reliquidación ({tranche.discountPercentage}% de la cuota)
                                    </span>
                                  )}
                                </td>
                              </tr>
          ...
          {/* Botón Nuevo Indicador debajo del Resumen Global (Indigo Style Image 4) */}
          <div className="flex items-center justify-start">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors shadow-sm cursor-pointer"
            >
          ...
          <div className="block sm:hidden space-y-3">
            {programIndicators.map((ind) => {
              ...
              const weight = ind.pesoRelativo ?? (programIndicators.length > 0 ? Math.round((100 / programIndicators.length) * 10) / 10 : 0);
              const weightedFinalResult = (cutResult * weight) / 100;
              const weightedTarget = (cutTarget * weight) / 100;
                ...
                >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {ind.pesoRelativo !== undefined && (
                          <span className="text-[11px] font-bold text-indigo-800 bg-indigo-100/70 border border-indigo-200/60 px-2 py-0.5 rounded-md">
                            Peso Relativo {ind.pesoRelativo}%
                          </span>
                        )}
                      </div>
                  ...
                  <div className="space-y-1.5 pt-1">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-700 font-bold text-[11px]">Cumplimiento</span>
                      <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-indigo-600 text-white font-black text-xs shadow-xs">
                        {Number(weightedFinalResult.toFixed(2))}%
                      </span>
                    </div>
                    <ProgressBar
                      value={percent}
                    ...
                    <div className="text-[11px] font-medium text-slate-600 pt-0.5">
                      {ind.direction === 'higher_is_better' ? (
                        isGapPositive ? (
                          <span>Faltan {formattedGap} {ind.unit}</span>
                        ) : (
                          <span>Cumplida (+{formattedGap} {ind.unit})</span>
                        )
                      ) : (
                        isGapPositive ? (
                          <span>Exceso +{formattedGap} {ind.unit}</span>
                        ) : (
                          <span>En meta ({cutResult} {ind.unit})</span>
                        )
                      )}
                    </div>
                  </div>
          ...
          <div className="hidden sm:block rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-100/60 font-semibold text-slate-700">
                    <th className="p-3.5">Indicador</th>
                    <th className="p-3.5">Nombre del Indicador</th>
                    <th className="p-3.5 text-center">Peso Relativo</th>
                    <th className="p-3.5 text-center">Meta Anual</th>
                    <th className="p-3.5 text-center">
                      {selectedCut === 'corte1' ? 'Meta 1° Corte' : selectedCut === 'corte2' ? 'Meta 2° Corte' : 'Meta 3° Corte'}
                    </th>
                    <th className="p-3.5 text-center">Resultado</th>
                    <th className="p-3.5">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-600 text-white font-bold text-xs shadow-xs">
                ...
                <tbody className="divide-y divide-slate-100">
                  {programIndicators.map((ind) => {
                    ...
                    const weight = (typeof ind.pesoRelativo === 'number' && !isNaN(ind.pesoRelativo) && ind.pesoRelativo > 0)
                      ? ind.pesoRelativo
                      : (programIndicators.length > 0 ? Math.round((100 / programIndicators.length) * 10) / 10 : 0);
                    const weightedFinalResult = Number.isFinite((cutResult * weight) / 100)
                      ? Number(((cutResult * weight) / 100).toFixed(2))
                      : 0;
                    ...
                    return (
                      <tr key={ind.id} className="hover:bg-slate-50/70 transition-colors">
                        ...
                        <td className="p-3.5 text-center font-bold text-indigo-700">
                          <span className="bg-indigo-50 text-indigo-700 px-2.5 py-1 rounded-lg text-xs border border-indigo-100 font-bold">
                            {weight}%
                          </span>
                        </td>
                        <td className="p-3.5 text-center font-medium text-slate-700">
                          {ind.annualTarget !== undefined && ind.annualTarget !== null && !isNaN(Number(ind.annualTarget)) ? `${ind.annualTarget} ${ind.unit || '%'}` : (cutTarget > 0 ? `${cutTarget} ${ind.unit || '%'}` : '—')}
                        </td>
                        <td className="p-3.5 text-center font-medium text-slate-700">
                          {cutTarget > 0 ? `${cutTarget} ${ind.unit || '%'}` : '—'}
                        </td>
                        <td className="p-3.5 text-center font-bold text-slate-900">
                          {Number.isFinite(cutResult) ? `${cutResult} ${ind.unit || '%'}` : '0 %'}
                        </td>
                        <td className="p-3.5">
                          <div className="flex items-center gap-2.5 w-32">
                            <ProgressBar
                              ...
                              size="sm"
                            />
                            <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-indigo-600 text-white font-black text-xs shadow-xs shrink-0">
                              {Number.isFinite(weightedFinalResult) ? Number(weightedFinalResult.toFixed(1)) : 0}%
                            </span>
                          </div>
                        </td>
                        <td className="p-3.5 font-medium text-slate-700">
                          {cutTarget > 0 ? (
                            ...
                            <span className="text-slate-400">Sin datos de corte</span>
                          )}
                        </td>
          ...
          {(() => {
            ...
            return (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                {/* 3. Presupuesto disponible (Indigo - Automático en tiempo real) */}
                <div className="p-4 sm:p-5 rounded-2xl border border-indigo-200 bg-indigo-50/50 shadow-xs space-y-2 transition-all">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-bold text-indigo-800 uppercase tracking-wider">
                      Presupuesto disponible
                    </span>
                    <span className="p-1.5 rounded-lg bg-indigo-100/80 text-indigo-700">
                      <DollarSign className="h-4 w-4" />
                    </span>
                  </div>

                  <div className="space-y-1">
                    <p className="text-2xl sm:text-3xl font-black text-indigo-700 tracking-tight font-mono">
                      ${liveAvailableNum.toLocaleString('es-CL')}
                    </p>
                    {isEditingBudgetCards && (
                      <span className="inline-flex items-center gap-1 text-[10px] font-bold text-indigo-700 bg-indigo-100 px-2 py-0.5 rounded-full border border-indigo-200 animate-pulse">
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        <thead>
                          <tr className="border-b border-slate-200 bg-slate-100/60 font-semibold text-slate-600">
                            ...
                            <th className="p-3 text-indigo-800">Saldo Disponible</th>
                        ...
                        <tfoot>
                          <tr className="border-t-2 border-slate-200 bg-slate-50 font-bold text-slate-800">
                            ...
                            <td className="p-3 text-indigo-800 font-mono font-black">
                          ...
                          >
                            ...
                            <div className="grid grid-cols-3 gap-2 text-xs pt-1 border-t border-slate-200">
                              ...
                              <div>
                                ...
                                <span className="font-bold text-indigo-700">${available.toLocaleString('es-CL')}</span>
                              </div>
                            </div>
      ...
      {activeTab === 'compras' && (() => {
        ...
        const getPurchaseCurrentCategory = (
          p: typeof programPurchases[0]
        ): 'pendiente' | 'en_ejecucion' | 'completado' => {
          const effectiveMacro = getPurchaseEffectiveMacroState(p);
          if (effectiveMacro === 'completado') {
            return 'completado';
          }
          if (effectiveMacro === 'en_ejecucion') {
            return 'en_ejecucion';
          }
          return 'pendiente';
        };

        const pendingCount = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'pendiente').length;
        const inProgressCount = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'en_ejecucion').length;
        const completedCount = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'completado').length;

        const pendientesPurchases = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'pendiente');
        const enEjecucionPurchases = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'en_ejecucion');
        const completadosPurchases = programPurchases.filter((p) => getPurchaseCurrentCategory(p) === 'completado');
        ...
        // Handler to move a purchase to any of the target categories in real-time
        const handleApplyPurchaseCategory = (
          purchaseId: string,
          targetCategory: 'todos' | 'pendiente' | 'en_ejecucion' | 'completado'
        ) => {
          if (targetCategory === 'todos') {
            setPurchaseFilter('todos');
            return;
          }
          const p = programPurchases.find((item) => item.id === purchaseId);
          if (!p) return;
          ...
          if (targetCategory === 'completado') {
            updatePurchase(purchaseId, {
              ...
              status: 'recepcionado',
              receptionStatus: p.receptionStatus === 'rechazada' ? 'rechazada' : 'conforme',
              receptionDate: p.receptionDate || todayIso,
            });
        ...
        const currentFilteredPurchases = programPurchases.filter((p) => {
          const currentCat = getPurchaseCurrentCategory(p);

          if (purchaseFilter !== 'todos') {
            if (purchaseFilter === 'pendiente' || purchaseFilter === 'por_hacer') {
              if (currentCat !== 'pendiente') return false;
            } else if (purchaseFilter === 'en_ejecucion') {
              if (currentCat !== 'en_ejecucion') return false;
            } else if (purchaseFilter === 'completado' || purchaseFilter === 'realizado') {
              if (currentCat !== 'completado') return false;
            }
          }

          if (purchaseSearchQuery.trim()) {
            const query = purchaseSearchQuery.toLowerCase();
            const matchItem = (p.itemOrService || '').toLowerCase().includes(query);
            const matchCategory = (p.category || '').toLowerCase().includes(query);
            const matchSupplier = (p.supplier || '').toLowerCase().includes(query);
            const matchOc = (p.purchaseOrderNumber || '').toLowerCase().includes(query);
            if (!matchItem && !matchCategory && !matchSupplier && !matchOc) {
              return false;
            }
          }
        ...
        const activeCategoryTotal =
          purchaseFilter === 'pendiente' || purchaseFilter === 'por_hacer'
            ...
            ? completadosTotal
            : (pendientesTotal + enEjecucionTotal + completadosTotal);

        return (
          <div className="space-y-4">
            {/* Header with 4 Droppable Macrostate Tabs & Quick Stats */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
              {/* Botones Filtro de Macroestado con Soporte Drag & Drop */}
              <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-xl border border-slate-200/80 flex-wrap">
                {/* Botón Ver Todos */}
                <button
                  type="button"
                  id="tab-compras-todos"
                  onClick={() => setPurchaseFilter('todos')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverPurchaseCard !== 'todos') setDragOverPurchaseCard('todos');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverPurchaseCard(null);
                  }}
                  onDrop={(ev) => {
                    ev.preventDefault();
                    setDragOverPurchaseCard(null);
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverPurchaseCard === 'todos'
                      ? 'bg-indigo-100 text-indigo-900 ring-2 ring-indigo-400 scale-[1.03]'
                      : purchaseFilter === 'todos'
                      ? 'bg-white text-indigo-900 shadow-xs border border-indigo-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <span className="pointer-events-none">Todos</span>
                  <span className="pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-slate-200 text-slate-700">
                    {programPurchases.length}
                  </span>
                </button>

                {/* Tab Pendiente - Drop Target */}
                <button
                  type="button"
                  id="tab-compras-pendiente"
                  onClick={() => setPurchaseFilter(purchaseFilter === 'pendiente' ? 'todos' : 'pendiente')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverPurchaseCard !== 'pendiente') setDragOverPurchaseCard('pendiente');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverPurchaseCard(null);
                  ...
                  onDrop={(ev) => {
                    ev.preventDefault();
                    setDragOverPurchaseCard(null);
                    const id = ev.dataTransfer.getData('text/plain') || draggedPurchaseId;
                    if (id) handleApplyPurchaseCategory(id, 'pendiente');
                  }}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverPurchaseCard === 'pendiente'
                      ? 'bg-amber-100 text-amber-900 ring-2 ring-amber-400 scale-[1.03]'
                      : purchaseFilter === 'pendiente' || purchaseFilter === 'por_hacer'
                      ? 'bg-white text-amber-900 shadow-xs border border-amber-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <Clock className="h-3.5 w-3.5 text-amber-600 pointer-events-none" />
                  <span className="pointer-events-none">Pendientes</span>
                  <span
                    className={`pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                      purchaseFilter === 'pendiente' || purchaseFilter === 'por_hacer'
                ...
                {/* Tab En ejecución - Drop Target */}
                <button
                  type="button"
                  id="tab-compras-en-ejecucion"
                  onClick={() => setPurchaseFilter(purchaseFilter === 'en_ejecucion' ? 'todos' : 'en_ejecucion')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverPurchaseCard !== 'en_ejecucion') setDragOverPurchaseCard('en_ejecucion');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverPurchaseCard(null);
                  ...
                  onDrop={(ev) => {
                    ...
                    setDragOverPurchaseCard(null);
                    const id = ev.dataTransfer.getData('text/plain') || draggedPurchaseId;
                    if (id) handleApplyPurchaseCategory(id, 'en_ejecucion');
                  }}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverPurchaseCard === 'en_ejecucion'
                      ? 'bg-blue-100 text-blue-900 ring-2 ring-blue-400 scale-[1.03]'
                      : purchaseFilter === 'en_ejecucion'
                      ? 'bg-white text-blue-900 shadow-xs border border-blue-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <TrendingUp className="h-3.5 w-3.5 text-blue-600 pointer-events-none" />
                  <span className="pointer-events-none">En ejecución</span>
                  <span
                    className={`pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                      purchaseFilter === 'en_ejecucion'
                ...
                {/* Tab Cerrados - Drop Target */}
                <button
                  type="button"
                  id="tab-compras-completado"
                  onClick={() => setPurchaseFilter(purchaseFilter === 'completado' ? 'todos' : 'completado')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverPurchaseCard !== 'completado') setDragOverPurchaseCard('completado');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverPurchaseCard(null);
                  ...
                  onDrop={(ev) => {
                    ...
                    setDragOverPurchaseCard(null);
                    const id = ev.dataTransfer.getData('text/plain') || draggedPurchaseId;
                    if (id) handleApplyPurchaseCategory(id, 'completado');
                  }}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverPurchaseCard === 'completado'
                      ? 'bg-emerald-100 text-emerald-900 ring-2 ring-emerald-400 scale-[1.03]'
                      : purchaseFilter === 'completado' || purchaseFilter === 'realizado'
                      ? 'bg-white text-emerald-800 shadow-xs border border-emerald-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <CheckCircle2 className="h-3.5 w-3.5 text-emerald-600 pointer-events-none" />
                  <span className="pointer-events-none">Cerrados</span>
                  <span
                    className={`pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                      purchaseFilter === 'completado' || purchaseFilter === 'realizado'
              ...
              <div className="flex items-center gap-2">
                <div className="relative w-full sm:w-56">
                  ...
                  <input
                    ...
                    placeholder="Buscar ítem, proveedor u OC..."
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 transition-colors shadow-xs cursor-pointer shrink-0"
                >
            ...
            {/* Drag helper hint */}
            {draggedPurchaseId && (
              <div className="p-2.5 bg-indigo-50 border border-indigo-200 rounded-xl text-xs text-indigo-700 font-medium flex items-center gap-2 animate-pulse">
                <GripVertical className="h-4 w-4 shrink-0 text-indigo-500" />
                <span>Arrastra y suelta la compra sobre cualquiera de las pestañas <strong>(Pendientes, En ejecución, Completado)</strong> para moverla de estado en tiempo real.</span>
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs">
              <div className="flex items-center gap-2 text-slate-600">
                ...
                <span>
                  {purchaseFilter === 'todos' && 'Todas las compras del programa. Puedes cambiar el estado desde el menú o arrastrar la fila a cualquier pestaña.'}
              ...
              <div className="font-mono font-bold text-slate-700 text-[11px] shrink-0">
                Total {purchaseFilter === 'todos' ? 'general' : 'filtrado'}: ${activeCategoryTotal.toLocaleString('es-CL')}
              </div>
            </div>
            ...
            {currentFilteredPurchases.length === 0 ? (
              <div className="text-center py-10 bg-slate-50 rounded-2xl border border-dashed border-slate-200 space-y-1.5">
                <p className="text-xs font-bold text-slate-700">
                  {purchaseFilter === 'todos' && 'No hay compras registradas en este programa'}
                ...
                <p className="text-[11px] text-slate-500 max-w-md mx-auto">
                  {purchaseFilter === 'todos' && 'Haz clic en "Nueva compra" para registrar una adquisición para este programa.'}
                ...
                <div className="pt-2">
                  <button
                    type="button"
                    onClick={() => onOpenQuickCreate('purchase')}
                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold hover:bg-indigo-700 transition-colors shadow-xs cursor-pointer"
                  >
              ...
              <>
                ...
                <div className="block md:hidden space-y-3">
                  {currentFilteredPurchases.map((p) => {
                    ...
                    const effectiveMacro = getPurchaseEffectiveMacroState(p);
                    const isDragging = draggedPurchaseId === p.id;
                    ...
                    return (
                      <div
                        ...
                        onDragStart={(e) => {
                          ...
                          setDraggedPurchaseId(p.id);
                        }}
                        onDragEnd={() => {
                          setDraggedPurchaseId(null);
                          setDragOverPurchaseCard(null);
                        }}
                        className={`p-4 rounded-2xl border border-slate-200 bg-white shadow-xs space-y-3 transition-all ${
                          isDragging ? 'opacity-50 border-dashed border-indigo-400 scale-[0.99]' : 'hover:border-indigo-200'
                        }`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2">
                            <div className="cursor-grab active:cursor-grabbing text-slate-400 hover:text-indigo-600 pt-0.5" title="Arrastra para cambiar estado">
                          ...
                          {/* Menu dropdown de cambio de estado en tiempo real */}
                          <PurchaseStatusChip
                            category={getPurchaseCurrentCategory(p)}
                            onChange={(newCat) => handleApplyPurchaseCategory(p.id, newCat)}
                          />
                        </div>
                        ...
                        return (
                          <tr
                            ...
                            className={`hover:bg-slate-50 transition-all ${
                              isDragging ? 'opacity-50 bg-indigo-50/50' : ''
                            }`}
                          >
                            <td className="p-3.5 text-center pr-0">
                              <div
                                className="cursor-grab active:cursor-grabbing text-slate-300 hover:text-indigo-600 inline-flex p-1 rounded hover:bg-slate-100"
                                title="Arrastra esta compra hacia las pestañas (Pendiente, En ejecución, Completado)"
                              >
                                <GripVertical className="h-4 w-4" />
                              </div>
                            </td>
                            ...
                            <td className="p-3.5">
                              <div className="space-y-1.5">
                                {/* Estado Selector interactivo en tiempo real */}
                                <PurchaseStatusChip
                                  category={getPurchaseCurrentCategory(p)}
                                  onChange={(newCat) => handleApplyPurchaseCategory(p.id, newCat)}
                                />
      ...
      {activeTab === 'tareas' && (() => {
        ...
        ): 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido' => {
        ...
        const handleApplyTaskCategory = (
          taskId: string,
          targetCategory: 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido'
        ) => {
          ...
          if (targetCategory === 'completado') {
            ...
            updateTask(taskId, {
              status: 'en_ejecucion',
              ...(isPastOrToday ? { dueDate: '2026-08-30' } : {}),
            }, true);
          } else if (targetCategory === 'pendiente') {
            const isPastOrToday = task.dueDate && task.dueDate <= todayStr;
            updateTask(taskId, {
            ...
            updateTask(taskId, {
              dueDate: todayStr,
              status: isTaskCompleted(task.status) ? 'por_hacer' : task.status,
            }, true);
        ...
        // Filter logic matching operational categories
        const filteredTasks = programTasks.filter((t) => {
          const currentCat = getTaskCurrentCategory(t);
        ...
        const pendingCount = programTasks.filter((t) => getTaskCurrentCategory(t) === 'pendiente').length;
        const inProgressCount = programTasks.filter((t) => getTaskCurrentCategory(t) === 'en_ejecucion').length;
        const completedCount = programTasks.filter((t) => getTaskCurrentCategory(t) === 'completado').length;
        const dueTodayCount = programTasks.filter((t) => getTaskCurrentCategory(t) === 'vence_hoy').length;
        const overdueCount = programTasks.filter((t) => getTaskCurrentCategory(t) === 'vencido').length;

        return (
          <div className="space-y-4">
            {/* Top Bar with 6 Segmented Tabs & Action */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
              ...
              <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-xl border border-slate-200/80 w-fit flex-wrap">
                ...
                <button
                  ...
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverTaskCard !== 'todos') setDragOverTaskCard('todos');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverTaskCard === 'vence_hoy'
                      ? 'bg-orange-100 text-orange-900 ring-2 ring-orange-400 scale-[1.03]'
                      : taskFilter === 'vence_hoy'
                      ? 'bg-white text-orange-800 shadow-xs border border-orange-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <AlertCircle className="h-3.5 w-3.5 text-orange-600 pointer-events-none" />
                  <span className="pointer-events-none">Vence hoy</span>
                  <span
                    className={`pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                      taskFilter === 'vence_hoy'
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverTaskCard === 'vencido'
                      ? 'bg-rose-100 text-rose-900 ring-2 ring-rose-400 scale-[1.03]'
                      : taskFilter === 'vencido'
                      ? 'bg-white text-rose-800 shadow-xs border border-rose-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                  <AlertTriangle className="h-3.5 w-3.5 text-rose-600 pointer-events-none" />
                  <span className="pointer-events-none">Vencidos</span>
                  <span
                    className={`pointer-events-none px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                      taskFilter === 'vencido'
            ...
            {filteredTasks.length === 0 ? (
              ...
              <div className="space-y-2.5">
                {filteredTasks.map((t) => {
                  ...
                  const currentCategory = getTaskCurrentCategory(t);
                  ...
                  return (
                    ...
                    >
                      ...
                      {t.description && (
                        <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 text-xs text-slate-600 leading-relaxed">
                          {t.description}
                        </div>
                      )}
                    </div>
      ...
      {activeTab === 'reuniones' && (() => {
        ...
        // Categorization helper: maps each meeting to one of the canonical cards
        ...
        // Handler to move a meeting to any of the target categories in real-time
        const handleApplyMeetingCategory = (
          meetingId: string,
          targetCategory: 'todos' | 'pendiente' | 'en_ejecucion' | 'completado'
        ) => {
          if (targetCategory === 'todos') {
            setMeetingCommitmentFilter('todos');
            return;
          }
        ...
        const filteredMeetings = programMeetings.filter((m) => {
          const currentCat = getMeetingCurrentCategory(m);

          if (meetingCommitmentFilter !== 'todos') {
            if (meetingCommitmentFilter === 'pendiente') {
              if (currentCat !== 'pendiente') return false;
            } else if (meetingCommitmentFilter === 'en_ejecucion') {
              if (currentCat !== 'en_ejecucion') return false;
            } else if (meetingCommitmentFilter === 'completado') {
              if (currentCat !== 'completado') return false;
            }
          }
        ...
        // Compute counts for the categories
        const pendingCount = programMeetings.filter((m) => getMeetingCurrentCategory(m) === 'pendiente').length;
        const inProgressCount = programMeetings.filter((m) => getMeetingCurrentCategory(m) === 'en_ejecucion').length;
        const completedCount = programMeetings.filter((m) => getMeetingCurrentCategory(m) === 'completado').length;

        return (
          <div className="space-y-4">
            {/* Top Bar with Droppable Tabs & Action */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
              {/* Segmented Filter Pills (4 categories: Todos, Pendientes, En ejecución, Cerrados) */}
              <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-xl border border-slate-200/80 w-fit flex-wrap">
                {/* Botón Ver Todos */}
                <button
                  type="button"
                  id="tab-reuniones-todos"
                  onClick={() => setMeetingCommitmentFilter('todos')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverMeetingCard !== 'todos') setDragOverMeetingCard('todos');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                  ...
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverMeetingCard === 'todos'
                      ? 'bg-indigo-100 text-indigo-900 ring-2 ring-indigo-400 scale-[1.03]'
                      : meetingCommitmentFilter === 'todos'
                      ? 'bg-white text-indigo-900 shadow-xs border border-indigo-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
            ...
            {filteredMeetings.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredMeetings.map((m) => {
                  ...
                  const currentCategory = getMeetingCurrentCategory(m);
                  const isClosed = currentCategory === 'completado' || m.status === 'finalizada' || m.status === 'completado' || m.status === 'cerrado';
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Acuerdos tomados (Solo cuando la reunión está cerrada) */}
                      {isClosed && agreementsList.length > 0 && (
                      ...
                      {/* Compromisos (Solo cuando la reunión está cerrada) */}
                      {isClosed && commitmentsList.length > 0 && (
      ...
      {activeTab === 'correos' && (() => {
        ...
        const handleApplyEmailCategory = (
          ...
          if (targetCategory === 'cerrado') {
            updateEmail(emailId, { status: 'cerrado' }, true);
          } else if (targetCategory === 'vencido') {
        ...
        const filteredEmails = programEmails.filter((e) => {
          const currentCat = getEmailCurrentCategory(e, todayStr);

          if (emailStatusFilter !== 'todos' && currentCat !== emailStatusFilter) {
            return false;
          }
        ...
        // Compute counts for the 5 tabs
        const pendingCount = programEmails.filter((e) => getEmailCurrentCategory(e, todayStr) === 'pendiente').length;
        const inProgressCount = programEmails.filter((e) => getEmailCurrentCategory(e, todayStr) === 'en_gestion').length;
        const resolvedCount = programEmails.filter((e) => getEmailCurrentCategory(e, todayStr) === 'cerrado').length;
        const dueTodayCount = programEmails.filter((e) => getEmailCurrentCategory(e, todayStr) === 'vence_hoy').length;
        const overdueCount = programEmails.filter((e) => getEmailCurrentCategory(e, todayStr) === 'vencido').length;

        return (
          <div className="space-y-4">
            {/* Top Bar with 5 Segmented Tabs & Action */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-3.5 rounded-2xl border border-slate-200 shadow-xs">
              {/* Segmented Filter Pills (6 categories: Todos, Pendientes, En ejecución, Cerrados, Vence hoy, Vencidos) */}
              <div className="flex items-center gap-1.5 p-1 bg-slate-100/90 rounded-xl border border-slate-200/80 w-fit flex-wrap">
                {/* Botón Ver Todos */}
                <button
                  type="button"
                  id="tab-correos-todos"
                  onClick={() => setEmailStatusFilter('todos')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverCard !== 'todos') setDragOverCard('todos');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                ...
                <button
                  ...
                  onClick={() => setEmailStatusFilter(emailStatusFilter === 'pendiente' ? 'todos' : 'pendiente')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                ...
                <button
                  ...
                  onClick={() => setEmailStatusFilter(emailStatusFilter === 'en_gestion' ? 'todos' : 'en_gestion')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                ...
                <button
                  ...
                  onClick={() => setEmailStatusFilter(emailStatusFilter === 'cerrado' ? 'todos' : 'cerrado')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                ...
                <button
                  type="button"
                  id="tab-correos-vence-hoy"
                  onClick={() => setEmailStatusFilter(emailStatusFilter === 'vence_hoy' ? 'todos' : 'vence_hoy')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverCard !== 'vence_hoy') setDragOverCard('vence_hoy');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverCard(null);
                  ...
                  onDrop={(ev) => {
                    ...
                    setDragOverCard(null);
                    const id = ev.dataTransfer.getData('text/plain') || draggedEmailId;
                    if (id) handleApplyEmailCategory(id, 'vence_hoy');
                  }}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverCard === 'vence_hoy'
                      ? 'bg-orange-100 text-orange-900 ring-2 ring-orange-400 scale-[1.03]'
                      : emailStatusFilter === 'vence_hoy'
                      ? 'bg-white text-orange-800 shadow-xs border border-orange-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
                ...
                <button
                  type="button"
                  id="tab-correos-vencidos"
                  onClick={() => setEmailStatusFilter(emailStatusFilter === 'vencido' ? 'todos' : 'vencido')}
                  onDragOver={(ev) => {
                    ev.preventDefault();
                    ev.dataTransfer.dropEffect = 'move';
                    if (dragOverCard !== 'vencido') setDragOverCard('vencido');
                  }}
                  onDragLeave={(ev) => {
                    if (ev.currentTarget.contains(ev.relatedTarget as Node)) return;
                    setDragOverCard(null);
                  ...
                  onDrop={(ev) => {
                    ...
                    setDragOverCard(null);
                    const id = ev.dataTransfer.getData('text/plain') || draggedEmailId;
                    if (id) handleApplyEmailCategory(id, 'vencido');
                  }}
                  className={`inline-flex items-center gap-2 px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                    dragOverCard === 'vencido'
                      ? 'bg-rose-100 text-rose-900 ring-2 ring-rose-400 scale-[1.03]'
                      : emailStatusFilter === 'vencido'
                      ? 'bg-white text-rose-800 shadow-xs border border-rose-200/80'
                      : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/60'
                  }`}
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  type="button"
                  id="btn-nuevo-correo-tab"
                  onClick={() => onOpenQuickCreate('email')}
                  className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-semibold hover:bg-purple-700 transition-colors shadow-xs shrink-0 cursor-pointer"
                >
                  <Plus className="h-3.5 w-3.5" /> Agregar
                </button>
              </div>
            ...
            {filteredEmails.length === 0 ? (
              <div className="p-8 rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-center space-y-2">
                ...
                <button
                  onClick={() => onOpenQuickCreate('email')}
                  className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white rounded-lg text-xs font-semibold hover:bg-purple-700 transition-colors cursor-pointer"
                >
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          ...
                          {h.status === 'en_proceso_seleccion' ? 'En Selección' : 'Activo'}
                        </span>
                      </div>
      ...
      {/* 10. BASE DE CONOCIMIENTO Y CRITERIOS OPERATIVOS */}
      ...
      {/* CONTACTOS DEL PROGRAMA (Gestión Centralizada con Filtro Scoped) */}
      {activeTab === 'contactos' && (
        <ContactosView
          scopedProgramId={currentProgram.id}
          onOpenEntity={onOpenEntity}
        />
      )}

      {/* DOCUMENTOS DEL PROGRAMA (Gestión Centralizada con Filtro Scoped) */}
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full overflow-hidden flex flex-col max-h-[90vh]">
                ...
                return (
                  <div className="grid grid-cols-3 gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200 text-center">
                    ...
                    <div>
                      ...
                      <span className="text-xs font-bold text-indigo-700">${dis.toLocaleString('es-CL')}</span>
                    </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-end gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-3 gap-2 p-3 rounded-xl bg-slate-50 border border-slate-200 text-center">
                    ...
                    <div>
                      <span className="text-[10px] text-slate-500 font-semibold uppercase block">Disponible</span>
                      <span className="text-xs font-bold text-indigo-700">${available.toLocaleString('es-CL')}</span>
                    </div>
                    <div className="col-span-3 pt-1 border-t border-slate-200 flex items-center justify-between text-[11px]">
      ...
      {/* Modal to configure and view program cut tranches */}
      <CutTableModal
        isOpen={isCutTableModalOpen}
        onClose={() => setIsCutTableModalOpen(false)}
