...
export const ProgramDetailView: React.FC<{
  ...
  const handleOpenEditHR = (h: HRRecord) => {
    ...
    setHrForm({
      ...
      contractType: (h.contractType === 'Código del Trabajo' ? 'Contrata' : h.contractType) || 'Contrata',
  ...
  const programContacts = (contacts || []).filter(
    (c) => !c.archived && (c.programIds.length === 0 || c.programIds.includes(currentProgram.id))
  );
  const programDocuments = (documents || []).filter(
    (d) => !d.archived && (d.programIds.length === 0 || d.programIds.includes(currentProgram.id))
  );
  ...
  const programKnowledge = knowledge.filter(
    (k) =>
      !k.archived &&
      ((k.programIds &&
        (k.programIds.length === 0 ||
          k.programIds.includes(currentProgram.id) ||
          k.programIds.includes('all'))) ||
        k.programId === currentProgram.id)
  );
      ...
      {activeTab === 'indicadores' && (
        <div className="space-y-4">
          ...
          {(() => {
            ...
            const sumWeightedCompliance = items.reduce((acc, item) => acc + item.weightedFinalResult, 0);
            const sumTargetWeighted = items.reduce((acc, item) => acc + item.weightedTarget, 0);
            ...
            const formattedCompliance = Number(sumWeightedCompliance.toFixed(2));
