...
import { ProgramId, Task, Indicator, Purchase, PendingEmail, Question, Meeting, getPurchaseDateFieldLabel, isEmailOverdue, normalizeCommunicationStatus } from '../../types';
import {
  Flame,
  CheckCircle2,
  Clock,
...
import { ProgramBadge, PriorityChip, TrafficLightBadge, ProgressBar, CommunicationTypeBadge, CommunicationDueBadge, CommunicationDeadlineNotice } from '../common/UIComponents';
...
export const HoyView: React.FC<{
  ...
  const rawPendingEmails = emails.filter((e) => {
    const c = normalizeCommunicationStatus(e.status);
    return !e.archived && c !== 'cerrado' && c !== 'respondido';
  });
  const currentPendingEmails: PendingEmail[] = programFilter === 'all'
    ? rawPendingEmails
    : rawPendingEmails.filter((e) => e.programId === programFilter);

  const overdueEmails = currentPendingEmails.filter((e) => isEmailOverdue(e, '2026-08-15'));
  const todayEmails = currentPendingEmails.filter((e) => e.deadline === '2026-08-15' && !isEmailOverdue(e, '2026-08-15'));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <div className="flex items-center justify-between border-b border-rose-100 bg-rose-50/70 px-5 py-3.5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-rose-900">
                Urgente — Vencidos y Bloqueados ({currentOverdueTasks.length + currentProblemPurchases.length + overdueEmails.length})
              </h3>
            </div>
          ...
          <div className="p-4 space-y-3">
            {currentOverdueTasks.length === 0 && currentProblemPurchases.length === 0 && overdueEmails.length === 0 ? (
              <div className="p-6 text-center text-xs text-slate-400">
                <CheckCircle2 className="h-6 w-6 text-emerald-500 mx-auto mb-1" />
                No hay elementos vencidos, compras bloqueadas ni requerimientos fuera de plazo en este momento.
              </div>
            ) : (
              <>
                {/* Overdue Emails / Requerimientos Vencidos */}
                {overdueEmails.map((e) => (
                  <div
                    key={e.id}
                    className="p-3 rounded-xl border border-rose-300 bg-rose-50/50 hover:bg-rose-50/80 transition-all flex items-start justify-between gap-3 text-left"
                  >
                    <div className="space-y-1 flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <CommunicationTypeBadge type={e.type} />
                        <ProgramBadge programId={e.programId} />
                      </div>
                      <h4
                      ...
                      {e.requiredAction && (
                        <p className="text-[11px] font-semibold text-rose-900 line-clamp-1">
                          Acción requerida: {e.requiredAction}
                        </p>
                      )}
                    ...
                    <button
                      onClick={() => onOpenEntity('email', e.id)}
                      className="px-2.5 py-1.5 bg-white border border-rose-200 hover:border-rose-400 hover:bg-rose-100/50 rounded-lg text-xs font-bold text-rose-700 shrink-0 shadow-xs cursor-pointer"
                    >
                      Gestionar
                    </button>
                  </div>
                    ...
                    <div className="flex items-start gap-2.5 flex-1">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2 text-[11px] text-slate-500">
                          ...
                          <span>•</span>
                          <span className="text-rose-600 font-bold">Venció el {formatDate(t.dueDate)}</span>
                          <span>•</span>
                    ...
                    <div className="space-y-1 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-bold text-rose-700 bg-rose-100 px-2 py-0.5 rounded border border-rose-200">
                          {p.category || 'Insumos de rehabilitación'}
                        </span>
                        <span
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500">
                        ...
                        <span>•</span>
                        <span>{getPurchaseDateFieldLabel(p.modalidadCompra)}: {formatDate(p.requiredDate || p.orderAcceptedDate)}</span>
                        <span>•</span>
                        <span>Monto: ${((p.totalPriceWithTax ?? p.estimatedAmount) || 0).toLocaleString('es-CL')}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => onOpenEntity('purchase', p.id)}
        ...
        >
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-5 py-3.5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-800">
                Hoy ({currentTodayTasks.length + todayMeetings.length + todayEmails.length})
              </h3>
            </div>
          ...
          <div className="p-4 space-y-3">
            {currentTodayTasks.length === 0 && todayMeetings.length === 0 && todayEmails.length === 0 ? (
              <div className="p-6 text-center text-xs text-slate-400">
                <Calendar className="h-6 w-6 text-slate-300 mx-auto mb-1" />
                No hay vencimientos, requerimientos al límite ni reuniones programadas para hoy.
              </div>
            ) : (
              <>
                {/* Requerimientos que vencen hoy */}
                {todayEmails.map((e) => (
                  <div
                    key={e.id}
                    className="p-3 rounded-xl border border-amber-200 bg-amber-50/40 flex items-start justify-between gap-3 text-left"
                  >
                    <div className="space-y-1 flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <CommunicationTypeBadge type={e.type} />
                    ...
                    <button
                      onClick={() => onOpenEntity('email', e.id)}
                      className="px-2.5 py-1.5 bg-white border border-amber-300 hover:border-amber-400 hover:bg-amber-100/50 rounded-lg text-xs font-bold text-amber-800 shrink-0 shadow-xs cursor-pointer"
                    >
                      Gestionar
                    </button>
                  </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-500 pt-1">
                      <span>Corte: {formatDate(ind.cutoffDate)} • Resp: {ind.responsible}</span>
                      <button
                        onClick={() => onOpenEntity('indicator', ind.id)}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                  Consultas y Preguntas en Seguimiento ({currentQuestions.length})
                </span>
                <button
                  type="button"
              ...
              {currentQuestions.length === 0 ? (
                ...
                currentQuestions.slice(0, 4).map((q) => {
                  const isOverdue = q.followUpDate && q.followUpDate < '2026-08-15' && q.status !== 'resuelta' && q.status !== 'cerrada_sin_respuesta';
                  return (
                    <div
                      key={q.id}
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 truncate">
                          {isOverdue && (
                            <span className="px-1.5 py-0.5 rounded text-[9px] font-bold bg-rose-100 text-rose-700 uppercase shrink-0">
                              Vencido
                            </span>
                          )}
                          {q.isUrgent && (
                          ...
                          <span className="font-semibold text-slate-900 truncate">{q.question}</span>
                        </div>
                        <ProgramBadge programId={q.programId} />
                      </div>
                      <div className="flex items-center justify-between text-[11px] text-slate-500 mt-1 pt-1 border-t border-slate-100/80">
                        <span>Dirigida a: <strong className="text-slate-700">{q.responsible}</strong></span>
                        <span>{q.followUpDate ? `Seguimiento: ${formatDate(q.followUpDate)}` : (q.nextInstance || 'Por definir')}</span>
                      </div>
                    </div>
                  );
                })
