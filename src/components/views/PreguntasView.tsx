    ...
    <div className="space-y-4 text-left">
      ...
      {filteredQuestions.length === 0 ? (
        <div className="bg-white p-12 text-center rounded-2xl border border-dashed border-slate-200 space-y-3">
          ...
          <button
            onClick={() => onOpenQuickCreate('question')}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 transition-all shadow-xs"
          >
        ...
        <div className="space-y-3.5 w-full">
          {filteredQuestions.map((q) => {
            ...
            return (
              <div
                key={q.id}
                className={`w-full rounded-2xl border bg-white p-5 transition-all shadow-xs ${
                  isOverdue
              ...
              >
                <div className="space-y-3">
                  {/* Top Bar: Program, Category & Quick Urgency / Status */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-2 border-t border-slate-100 text-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-1.5 text-slate-600">
                        ...
                        <span className="font-medium text-indigo-700 truncate" title={q.nextInstance}>
                          {q.nextInstance || 'Por definir'}
                        </span>
                      </div>
                    </div>

                    {/* Fecha de Seguimiento & Alertas */}
                  ...
                  {/* Recent Follow-Up / Bitácora Preview */}
                    ...
                    {isLogsOpen ? (
                      <div className="space-y-2 mt-2 max-h-48 overflow-y-auto pr-1">
                        {recentFollowUps.length === 0 ? (
                          <p className="text-[11px] text-slate-400 italic py-1">Sin hitos registrados aún.</p>
                        ) : (
                      ...
                      recentFollowUps.length > 0 && (
                        <div className="p-2 rounded-lg bg-slate-50 text-[11px] text-slate-600 flex items-start justify-between gap-2">
                ...
                <div className="flex items-center justify-between gap-2 pt-4 mt-3 border-t border-slate-100">
                  <div className="flex items-center gap-1.5">
                    ...
                    {!isResolved && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-all cursor-pointer"
                        title="Registrar respuesta final y cerrar como resuelta"
                      >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg border border-rose-200 bg-rose-50/50 text-rose-600 hover:border-rose-300 hover:bg-rose-100 hover:text-rose-700 transition-all shadow-xs"
