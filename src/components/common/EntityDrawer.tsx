...
export const EntityDrawer: React.FC<{
  ...
  const [newQFuType, setNewQFuType] = useState<QuestionFollowUpType>('reiteracion');
  const [newQFuNote, setNewQFuNote] = useState('');
  const [newQAttName, setNewQAttName] = useState('');
  ...
  const [indMetaAnualCant, setIndMetaAnualCant] = useState('');
  const [indMetaAnualPorc, setIndMetaAnualPorc] = useState('');
  const [indMetaCorteCant, setIndMetaCorteCant] = useState('');
  const [indMetaCortePorc, setIndMetaCortePorc] = useState('');
  const [indCurrentCant, setIndCurrentCant] = useState('');
  ...
  const handleAddQuestionFollowUp = () => {
    if (!draftQuestion || !newQFuNote.trim()) return;
    const newFu: QuestionFollowUp = {
      id: `qfu_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
      questionId: draftQuestion.id,
      type: newQFuType,
      note: newQFuNote.trim(),
      createdAt: new Date().toISOString(),
      createdBy: currentUser.name,
    };
    setDraftQuestion({
      ...draftQuestion,
      followUps: [newFu, ...(draftQuestion.followUps || [])],
    });
    setNewQFuNote('');
  };

  const handleRemoveQuestionFollowUp = (fuId: string) => {
    if (!draftQuestion) return;
    setDraftQuestion({
      ...draftQuestion,
      followUps: (draftQuestion.followUps || []).filter((f) => f.id !== fuId),
    });
  };

  const handleAddQuestionAttachment = () => {
    if (!draftQuestion || !newQAttName.trim()) return;
    const newAtt: QuestionAttachment = {
      id: `qatt_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`,
      questionId: draftQuestion.id,
      name: newQAttName.trim(),
    ...
    setDraftQuestion({
      ...draftQuestion,
      attachments: [newAtt, ...(draftQuestion.attachments || [])],
    });
    setNewQAttName('');
  };

  const handleRemoveQuestionAttachment = (attId: string) => {
    if (!draftQuestion) return;
    setDraftQuestion({
      ...draftQuestion,
      attachments: (draftQuestion.attachments || []).filter((a) => a.id !== attId),
    });
  };
  ...
  useEffect(() => {
    if (indicator) {
      ...
      setIndNumTipo((indicator.numeradorTipo as any) || (indicator.unit === 'cantidad' ? 'cantidad' : 'porcentaje'));
      ...
      setIndDenTipo((indicator.denominadorTipo as any) || (indicator.unit === 'cantidad' ? 'cantidad' : 'porcentaje'));
      ...
      setIndMetaAnualCant(
        indicator.annualTargetQuantity !== undefined
          ? String(indicator.annualTargetQuantity)
          : indicator.unit === 'cantidad'
          ? String(indicator.annualTarget)
          : ''
      );
      setIndMetaAnualPorc(
      ...
      const activeCutData =
        indicator.corteSeleccionado === '2° corte'
          ? indicator.corte2
          : indicator.corteSeleccionado === '3° corte'
          ? indicator.corte3
          : indicator.corte1;

      setIndMetaCorteCant(
        activeCutData?.targetQuantity !== undefined
          ? String(activeCutData.targetQuantity)
          : indicator.periodTargetQuantity !== undefined
          ? String(indicator.periodTargetQuantity)
          : ''
      );
      setIndMetaCortePorc(
        activeCutData?.target !== undefined
          ? String(activeCutData.target)
          : String(indicator.periodTarget || indicator.annualTarget || 90)
      );

      setIndCurrentCant(
        indicator.currentResultQuantity !== undefined
          ? String(indicator.currentResultQuantity)
          : indicator.unit === 'cantidad'
          ? String(indicator.currentResult)
          : ''
      );
      setIndCurrent(String(indicator.currentResult ?? 0));
      setIndFechaCorte(activeCutData?.date || indicator.cutoffDate || '2026-08-15');
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-6">
              ...
              <div className="lg:col-span-7 space-y-4">
                <div className="bg-slate-50/70 border border-slate-200 rounded-2xl p-3.5 sm:p-5 space-y-3.5 sm:space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                    <div className="sm:col-span-3">
                      ...
                      >
                        ...
                        {indCode && !indCode.startsWith('Indicador ') && (
                  ...
                  {/* Metas y Resultados en Cantidad y % */}
                  <div className="space-y-3 bg-slate-50 p-3 sm:p-4 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          <div className="relative">
                            <input
                              ...
                              step="any"
                              placeholder="Cant."
                              value={indMetaAnualCant}
                              onChange={(e) => setIndMetaAnualCant(e.target.value)}
                      ...
                      <div className="space-y-1.5 bg-white p-2.5 rounded-xl border border-slate-200 shadow-2xs">
                        <label className="block text-[11px] font-bold text-indigo-900 truncate" title={`Meta para este Corte (${indCorte})`}>
                          Meta para este Corte ({indCorte}) *
                        </label>
                        <div className="grid grid-cols-2 gap-1.5">
                          <div className="relative">
                            <input
                              ...
                              step="any"
                              placeholder="Cant."
                              value={indMetaCorteCant}
                              onChange={(e) => setIndMetaCorteCant(e.target.value)}
                              className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 pr-6 text-xs font-semibold text-slate-800 focus:border-indigo-500 focus:outline-none"
                            />
                            <span className="absolute right-1.5 top-1.5 text-[10px] font-bold text-slate-400">N°</span>
                          </div>
                          <div className="relative">
                            <input
                              ...
                              required
                              placeholder="%"
                              value={indMetaCortePorc}
                              onChange={(e) => setIndMetaCortePorc(e.target.value)}
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          <div className="relative">
                            <input
                              ...
                              step="any"
                              placeholder="Cant."
                              value={indCurrentCant}
                              onChange={(e) => setIndCurrentCant(e.target.value)}
                              className="w-full rounded-lg border border-slate-300 bg-white px-2 py-1.5 pr-6 text-xs font-semibold text-slate-800 focus:border-indigo-500 focus:outline-none"
                            />
                            <span className="absolute right-1.5 top-1.5 text-[10px] font-bold text-slate-400">N°</span>
                          </div>
                  ...
                  {(() => {
                    ...
                    const indicatorLabel = indCode || 'Indicador 1';
                      ...
                      {entityAttachments.map((att) => (
                        <div key={att.id} className="p-2 border border-slate-200 rounded-xl flex items-center justify-between text-xs bg-slate-50">
                          ...
                          <span className="text-[10px] text-slate-400 shrink-0 ml-2">
                            {(((typeof att.size === 'number' ? att.size : parseFloat(att.size as any)) || 0) / 1024).toFixed(0)} KB
                          </span>
                        </div>
                      ))}
                    </div>
        ...
        <div className="p-4 sm:p-5 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500">
              Detalle de {
                ...
                (entityType as string) === 'budget' ? 'Presupuesto' :
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 text-left">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="ej. Sala de Reuniones DISAM Quilicura"
                          className="w-full rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-800 focus:ring-2 focus:ring-indigo-500"
                        />
                      </div>
                    </div>

                    {/* GOOGLE CALENDAR & VIDEO CALL INTEGRATION */}
                    {(() => {
                      const video = extractVideoMeetingLink(curMeeting.meetingLink || curMeeting.location || curMeeting.summary || curMeeting.description);
                      const webGCalUrl = generateGoogleCalendarWebUrl({
                        title: curMeeting.title || 'Reunión Quilicura',
                        description: curMeeting.summary || curMeeting.description || '',
                        location: curMeeting.location || curMeeting.meetingLink || '',
                        startDateTime: curMeeting.dateTime,
                      });

                      return (
                        <div className="p-3.5 rounded-xl border border-indigo-200 bg-indigo-50/70 space-y-2.5">
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                              <CalendarCheck2 className="h-4 w-4 text-indigo-600 shrink-0" />
                              <span className="text-xs font-bold text-indigo-950">
                                Google Calendar & Videollamada (Zoom / Meet)
                              </span>
                            </div>
                            <div className="flex items-center gap-2">
                              {curMeeting.googleCalendarEventId ? (
                                <span className="text-[10px] font-bold bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-full flex items-center gap-1">
                                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-500" />
                                  Sincronizado con GCal
                                </span>
                              ) : (
                                <button
                                  type="button"
                                  onClick={async () => {
                                    const ok = await exportMeetingToGoogleCalendar(curMeeting);
                                    if (ok) {
                                      updateDraftMeeting({ source: 'gcal' });
                                    }
                                  }}
                                ...
                                >
                                  ...
                                  <span>Vincular a Google Calendar</span>
                                </button>
                              )}

                              <a
                                href={curMeeting.googleCalendarHtmlLink || webGCalUrl}
                                target="_blank"
                                rel="noreferrer"
                                className="px-2 py-1 rounded-lg bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 text-xs font-semibold flex items-center gap-1 transition-colors"
                              >
                          ...
                          <div>
                            ...
                            <div className="flex gap-2">
                              <input
                                type="url"
                                placeholder="https://meet.google.com/... ó https://zoom.us/j/..."
                                value={curMeeting.meetingLink || ''}
                                onChange={(e) => updateDraftMeeting({ meetingLink: e.target.value })}
                                className="flex-1 rounded-lg border border-slate-300 bg-white px-2.5 py-1.5 text-xs text-slate-800 font-mono focus:ring-2 focus:ring-indigo-500"
                              />
                              {video && (
                                <a
                                  href={video.url}
                                  target="_blank"
                                  rel="noreferrer"
                                  className={`px-3 py-1.5 rounded-lg text-xs font-bold text-white flex items-center gap-1.5 shrink-0 transition-colors ${
                                    video.platform === 'meet'
                                      ? 'bg-emerald-600 hover:bg-emerald-700'
                                      : video.platform === 'zoom'
            ...
            {question && (() => {
              ...
              const isOverdue = isQuestionOverdue(curQ, todayStr);
              const isToday = isQuestionDueToday(curQ, todayStr);
              const linkedTask = curQ.taskId ? tasks.find((t) => t.id === curQ.taskId) : null;
              const followUps = curQ.followUps || [];
              const attachmentsList = curQ.attachments || [];
              ...
              return (
                <div className="space-y-6">
                  ...
                  <div className="p-4 rounded-2xl border border-slate-200 bg-slate-50/70 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <QuestionStatusChip
                          status={curQ.status}
                          onChange={(newStatus) => updateDraftQuestion({ status: newStatus })}
                        />
                      </div>

                      <div className="flex items-center gap-2">
                        <QuestionCategoryBadge category={curQ.category} />
                        <PriorityChip priority={curQ.priority} />
                      </div>
                    </div>
                      ...
                      <div className="p-2.5 rounded-xl bg-rose-50 border border-rose-200 text-xs text-rose-800 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-bold">
                            Seguimiento Vencido: Plazo de gestión fue el {formatDate(curQ.followUpDate)}
                          </span>
                        </div>
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-rose-200 text-rose-800">
                          Vencido
                    ...
                    {isToday && (
                      <div className="p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-800 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-amber-600 shrink-0" />
                          <span className="font-bold">
                            El seguimiento de esta consulta debe realizarse hoy ({formatDate(curQ.followUpDate)})
                          </span>
                        </div>
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase bg-amber-200 text-amber-800">
                          Vence hoy
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                      Consulta / Duda Operativa *
                    </label>
                    <input
                      type="text"
                      value={curQ.question || ''}
                      onChange={(e) => updateDraftQuestion({ question: e.target.value })}
                      placeholder="ej. ¿Cuál es el criterio para la redistribución de fondos del componente B?"
                      className="w-full rounded-xl border border-slate-300 bg-white p-3 text-sm font-bold text-slate-900 focus:ring-2 focus:ring-indigo-500 shadow-2xs"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 mb-1.5">
                      Contexto y Justificación del Problema *
                    </label>
                    <textarea
                      rows={3}
                      value={curQ.context || ''}
                      onChange={(e) => updateDraftQuestion({ context: e.target.value })}
                      placeholder="Explica el origen de la duda, normativa involucrada o necesidad operativa..."
                      className="w-full rounded-xl border border-slate-300 bg-white p-3 text-xs text-slate-800 focus:ring-2 focus:ring-indigo-500 shadow-2xs"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                    ...
                    <div>
                      ...
                      <select
                        value={curQ.category}
                        onChange={(e) => updateDraftQuestion({ category: e.target.value as QuestionCategory })}
                        className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500"
                      >
                        <option value="tecnica">Técnica / Clínica</option>
                        <option value="normativa">Normativa / Legal</option>
                        <option value="administrativa">Administrativa / Flujo</option>
                        <option value="financiera">Financiera / Presupuesto</option>
                        <option value="servicio_salud">Servicio de Salud SSMN</option>
                        <option value="gestion_interna">Gestión Interna / Comunal</option>
                    ...
                    <div>
                      ...
                      <select
                        value={curQ.programId}
                        onChange={(e) => updateDraftQuestion({ programId: e.target.value as any })}
                        className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500"
                      >
                        {programs.map((p) => (
                          <option key={p.id} value={p.id}>
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={curQ.followUpDate || ''}
                        onChange={(e) => updateDraftQuestion({ followUpDate: e.target.value })}
                        className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs text-slate-800 focus:ring-2 focus:ring-indigo-500"
                      />
                    </div>
                    ...
                    <div className="sm:col-span-2">
                      <label className="block text-slate-600 font-semibold mb-1">
                        Próxima Instancia de Resolución
                      </label>
                      <input
                        type="text"
                        value={curQ.nextInstance || ''}
                        onChange={(e) => updateDraftQuestion({ nextInstance: e.target.value })}
                        placeholder="ej. Mesa técnica mensual con SSMN / Reunión bilateral"
                        className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs text-slate-800 focus:ring-2 focus:ring-indigo-500"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        value={curQ.priority}
                        onChange={(e) => updateDraftQuestion({ priority: e.target.value as PriorityLevel })}
                        className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs font-semibold text-slate-800 focus:ring-2 focus:ring-indigo-500"
                      >
                        <option value="critica">Crítica</option>
                        <option value="alta">Alta</option>
                    ...
                    <div className="sm:col-span-2 flex items-center gap-3 pt-5">
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={curQ.isUrgent || false}
                          onChange={(e) => updateDraftQuestion({ isUrgent: e.target.checked })}
                  ...
                  <div className="p-4 rounded-xl bg-emerald-50/60 border border-emerald-200 space-y-3">
                    ...
                    <textarea
                      ...
                      placeholder="Escriba la orientación definitiva o acuerdo emanado de la contraparte / SSMN..."
                      className="w-full rounded-xl border border-emerald-300/80 bg-white p-3 text-xs text-slate-800 focus:ring-2 focus:ring-emerald-500 shadow-2xs"
                    />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                      <div>
                        ...
                        <input
                          type="text"
                          value={curQ.sourceOfResponse || ''}
                          onChange={(e) => updateDraftQuestion({ sourceOfResponse: e.target.value })}
                          placeholder="ej. Referente Técnico SSMN / Jurídico DISAM"
                          className="w-full rounded-lg border border-emerald-300 bg-white p-2 text-xs text-slate-800 focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>

                      {curQ.status === 'cerrada_sin_respuesta' && (
                        <div>
                          ...
                          <input
                            type="text"
                            value={curQ.closedReason || ''}
                            onChange={(e) => updateDraftQuestion({ closedReason: e.target.value })}
                            placeholder="ej. Duda desistida por cambio de normativa..."
                            className="w-full rounded-lg border border-rose-300 bg-white p-2 text-xs text-slate-800 focus:ring-2 focus:ring-rose-500"
                          />
                        </div>
                      )}
                    </div>
                  ...
                  {/* Follow-ups & Milestones Timeline (Bitácora) */}
                  <div className="pt-3 border-t border-slate-200 space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                        <MessageSquare className="h-4 w-4 text-indigo-600" />
                        Bitácora de Hitos de Seguimiento ({followUps.length})
                      </label>
                      <span className="text-[11px] text-slate-500">
                        {followUps.length} {followUps.length === 1 ? 'hito registrado' : 'hitos registrados'}
                      </span>
                    ...
                    {followUps.length > 0 ? (
                      <div className="space-y-2">
                        {followUps.map((fu) => {
                          const fuTypeLabels: Record<string, { label: string; bg: string; text: string }> = {
                            consulta_enviada: { label: 'Consulta Enviada', bg: 'bg-blue-50', text: 'text-blue-700' },
                            reiteracion: { label: 'Reiteración / Insistencia', bg: 'bg-rose-50', text: 'text-rose-700' },
                            respuesta_parcial: { label: 'Respuesta Preliminar', bg: 'bg-amber-50', text: 'text-amber-700' },
                            aclaracion_verbal: { label: 'Aclaración Verbal', bg: 'bg-purple-50', text: 'text-purple-700' },
                            en_reunion: { label: 'Tratado en Reunión', bg: 'bg-indigo-50', text: 'text-indigo-700' },
                            nota_interna: { label: 'Nota Interna', bg: 'bg-slate-100', text: 'text-slate-700' },
                          };
                          const conf = fuTypeLabels[fu.type] || fuTypeLabels.nota_interna;
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center justify-between gap-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleRemoveQuestionFollowUp(fu.id)}
                                  className="text-slate-400 hover:text-rose-600 p-1 rounded-md transition-colors cursor-pointer"
                                  title="Eliminar hito"
                                >
                                  <Trash2 className="h-3.5 w-3.5" />
                      ...
                      <p className="text-xs text-slate-400 italic p-3 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-center">
                        Sin hitos registrados aún en la bitácora de la consulta.
                      </p>
                    )}
                    ...
                    <div className="p-3 rounded-xl border border-indigo-100 bg-indigo-50/40 space-y-2.5">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <div>
                          <select
                            value={newQFuType}
                            onChange={(e) => setNewQFuType(e.target.value as QuestionFollowUpType)}
                            className="w-full rounded-lg border border-slate-300 bg-white p-2 text-xs font-semibold text-slate-800"
                          >
                            <option value="reiteracion">⏰ Reiteración</option>
                            <option value="respuesta_parcial">💬 Rpta. Preliminar</option>
                          </select>
                        </div>
                        <div className="sm:col-span-2 flex gap-2">
                          <input
                            type="text"
                            value={newQFuNote}
                            onChange={(e) => setNewQFuNote(e.target.value)}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                e.preventDefault();
                                handleAddQuestionFollowUp();
                              }
                            }}
                            placeholder="Describa el avance, llamada o respuesta recibida..."
                            className="flex-1 rounded-lg border border-slate-300 bg-white p-2 text-xs text-slate-800"
                          ...
                          <button
                            type="button"
                            onClick={handleAddQuestionFollowUp}
                            disabled={!newQFuNote.trim()}
                            className="px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-bold hover:bg-indigo-700 disabled:opacity-50 transition-colors shrink-0 cursor-pointer"
                          >
                            Agregar Hito
                          </button>
                  ...
                  <div className="pt-3 border-t border-slate-200 space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-bold text-slate-800 uppercase tracking-wider flex items-center gap-1.5">
                        ...
                        Archivos Adjuntos y Documentos de Respaldo ({attachmentsList.length})
                      </label>
                    </div>
                    ...
                    {attachmentsList.length > 0 ? (
                        ...
                        {attachmentsList.map((att) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <button
                                type="button"
                                onClick={() => handleRemoveQuestionAttachment(att.id)}
                                className="p-1 text-slate-400 hover:text-rose-600 rounded transition-colors"
                              >
                                <Trash2 className="h-3.5 w-3.5" />
                              </button>
                      ...
                      <p className="text-xs text-slate-400 italic p-3 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-center">
                        Sin archivos adjuntos a esta consulta.
                      </p>
                    )}
                    ...
                    <div className="flex gap-2">
                      <input
                        type="text"
                        value={newQAttName}
                        onChange={(e) => setNewQAttName(e.target.value)}
                        placeholder="Nombre del archivo (ej. Oficio_Respuesta_SSMN.pdf)..."
                        className="flex-1 rounded-lg border border-slate-300 bg-white p-2 text-xs text-slate-800"
                      />
                      <button
                        type="button"
                        onClick={handleAddQuestionAttachment}
                        disabled={!newQAttName.trim()}
                        className="px-3 py-2 bg-slate-800 text-white rounded-lg text-xs font-bold hover:bg-slate-900 disabled:opacity-50 transition-colors shrink-0"
                      >
                        Adjuntar
                      </button>
                    </div>
            ...
            {/* ATTACHMENTS MANAGER FOR ANY ENTITY (EXCEPT QUESTION WHICH HAS ITS OWN DEDICATED ATTACHMENTS MANAGER) */}
            {entityType !== 'question' && (() => {
