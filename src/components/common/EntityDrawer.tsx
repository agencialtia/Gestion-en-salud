          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 text-left">
            ...
            {task && (() => {
              ...
              return (
                <div className="space-y-5">
                  <div>
                    <h2 className="text-lg font-bold text-slate-900">{curTask.title}</h2>
                    {curTask.description && (
                  ...
                  <div className="grid grid-cols-2 gap-3 p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => updateDraftTask({ status: e.target.value as TaskStatus })}
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-2 bg-emerald-600 text-white rounded-xl text-xs font-semibold hover:bg-emerald-700 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                    >
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    {formatDateTime(meeting.dateTime)} • {meeting.location}
                  </p>
                </div>
                    ...
                    {meeting.commitments.map((c) => (
                      <div key={c.id} className="p-3 border border-slate-200 rounded-xl space-y-1.5 bg-white">
                        ...
                        <div className="flex items-center justify-between text-[11px] text-slate-500">
                          <span>Resp: {c.responsible} • Plazo: {formatDate(c.deadline)}</span>
                          {c.taskId ? (
            ...
            {email && (() => {
              ...
              return (
                <div className="space-y-5">
                  <div>
                    <h2 className="text-base font-bold text-slate-900">{curEmail.subject}</h2>
                    <p className="text-xs text-slate-500 mt-0.5">
                      Destinatario: {curEmail.recipient} • Fecha recepción: {formatDate(curEmail.receivedDate)}
                    </p>
                  </div>

                  <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-2">
                    <p className="text-slate-700 leading-relaxed">{curEmail.summary}</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      ...
                      <select
                        value={curEmail.status}
                        onChange={(e) => updateDraftEmail({ status: e.target.value as EmailStatus })}
