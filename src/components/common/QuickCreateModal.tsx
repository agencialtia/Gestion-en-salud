...
export const QuickCreateModal: React.FC<{ 
  ...
  const [taskCategory, setTaskCategory] = useState('Gestión');
  const [taskIsUrgent, setTaskIsUrgent] = useState(false);
  const [taskDesc, setTaskDesc] = useState('');

  // Meeting form state
  const [meetTitle, setMeetTitle] = useState('');
  const [meetType, setMeetType] = useState<MeetingType>('reunion');
  const [meetStatus, setMeetStatus] = useState<MeetingStatus>('programada');
  ...
  const [meetSyncGCal, setMeetSyncGCal] = useState(true);
  ...
  const [meetAgreementsText, setMeetAgreementsText] = useState('');
  const [meetCommitmentDesc, setMeetCommitmentDesc] = useState('');
  const [meetCommitmentResp, setMeetCommitmentResp] = useState(currentUser.name);
  const [meetCommitmentDeadline, setMeetCommitmentDeadline] = useState('2026-08-25');
  const [meetCommitmentIsUrgent, setMeetCommitmentIsUrgent] = useState(false);
  const [meetCommitmentsList, setMeetCommitmentsList] = useState<{ description: string; responsible: string; deadline: string; isUrgent: boolean }[]>([]);

  // Purchase form state
  const [purMacroState, setPurMacroState] = useState<PurchaseMacroState>('pendiente');
  ...
  const [purCeroPapelInitiationDate, setPurCeroPapelInitiationDate] = useState('2026-08-15');
  ...
  const [emailSender, setEmailSender] = useState('Klaus Bauer (DISAM)');
  const [emailRecipient, setEmailRecipient] = useState('');
  const [emailResponsible, setEmailResponsible] = useState('Klaus Bauer');
  const [emailType, setEmailType] = useState<CommunicationType>('requerimiento');
  ...
  const [emailIsUrgent, setEmailIsUrgent] = useState(true);
  const [emailRequiredAction, setEmailRequiredAction] = useState('');
  ...
  const [questionPriority, setQuestionPriority] = useState<PriorityLevel>('alta');
  const [questionIsUrgent, setQuestionIsUrgent] = useState(false);
  const [questionResponsible, setQuestionResponsible] = useState(currentUser.name);
  const [questionFollowUpDate, setQuestionFollowUpDate] = useState('2026-08-20');
  const [questionNextInst, setQuestionNextInst] = useState('');
  const [questionForNextMeeting, setQuestionForNextMeeting] = useState(false);
  ...
  const [indCodigo, setIndCodigo] = useState('Indicador 1');
  ...
  const [indNumTipo, setIndNumTipo] = useState<'porcentaje' | 'cantidad'>('porcentaje');
  const [indDenDesc, setIndDenDesc] = useState('');
  const [indDenPorc, setIndDenPorc] = useState('');
  const [indDenTipo, setIndDenTipo] = useState<'porcentaje' | 'cantidad'>('porcentaje');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'task') {
      ...
      const agreementsParsed = meetAgreementsText
        .split('\n')
        .map((line) => line.trim())
        .filter((line) => line.length > 0)
        .map((line, idx) => ({
          id: `agr_qc_${Date.now()}_${idx}`,
          meetingId: '',
          description: line,
        }));

      const commitmentsParsed = meetCommitmentsList.map((c, idx) => ({
        id: `com_qc_${Date.now()}_${idx}`,
        meetingId: '',
        description: c.description,
        responsible: c.responsible,
        deadline: c.deadline,
        priority: c.isUrgent ? ('critica' as PriorityLevel) : ('media' as PriorityLevel),
        status: 'pendiente' as const,
        isUrgent: c.isUrgent,
      }));

      const createdMeeting = addMeeting({
        title: meetTitle,
        programId: selectedProgram,
        type: meetType,
        status: meetStatus,
        ...
        meetingLink: meetLink.trim() || undefined,
        description: meetDescription.trim() || undefined,
        summary: meetDescription.trim() || meetObjective.trim() || undefined,
        participants: meetParticipants.split(',').map((s) => s.trim()).filter((s) => s.length > 0),
        objective: meetObjective,
        notes: '',
        agreements: agreementsParsed.length > 0 ? agreementsParsed : [],
        commitments: commitmentsParsed,
        source: 'local',
      });

      if (meetSyncGCal || isGoogleCalendarConnected) {
        exportMeetingToGoogleCalendar(createdMeeting);
      }
      ...
      const createdPur = addPurchase({
        ...
        ceroPapelInitiationDate: purCeroPapelInitiationDate || '2026-08-15',
        ...
        receptionStatus: purMacroState === 'completado' ? purReceptionStatus : 'pendiente',
        receptionDate: purMacroState === 'completado' ? (purReceptionDate || undefined) : undefined,
        receptionResponsible: purMacroState === 'completado' ? (purReceptionResponsible || undefined) : undefined,
        receptionActDoc: purMacroState === 'completado' ? (purReceptionActDoc || undefined) : undefined,
        receptionNotes: purMacroState === 'completado' ? (purReceptionNotes || undefined) : undefined,
        invoiceStatus: purMacroState === 'completado' ? purInvoiceStatus : 'sin_factura',
        invoiceNumber: purMacroState === 'completado' ? (purInvoiceNumber || undefined) : undefined,
        invoiceDate: purMacroState === 'completado' ? (purInvoiceDate || undefined) : undefined,
        invoiceAmount: purMacroState === 'completado' && purInvoiceAmount ? parseFloat(purInvoiceAmount) : undefined,
        invoicePaymentDate: purMacroState === 'completado' ? (purInvoicePaymentDate || undefined) : undefined,
        invoiceNotes: purMacroState === 'completado' ? (purInvoiceNotes || undefined) : undefined,
      });
      ...
      addEmail({
        ...
        programId: selectedProgram,
        responsible: emailResponsible,
        type: emailType,
        action: emailAction,
        priority: emailIsUrgent ? 'alta' : emailPriority,
        isUrgent: emailIsUrgent,
        requiredAction: emailRequiredAction,
        notes: emailNotes,
      ...
      addQuestion({
        ...
        responsible: questionResponsible.trim() || currentUser.name,
        priority: questionPriority,
        isUrgent: questionIsUrgent,
        status: 'pendiente',
        followUpDate: questionFollowUpDate,
        nextInstance: questionNextInst.trim(),
        forNextMeeting: questionForNextMeeting,
      });
        ...
        <form onSubmit={handleSubmit} noValidate className="p-3.5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          {activeTab === 'task' && (
            <>
              ...
              <div className="flex items-center gap-2 py-1">
                <button
                  type="button"
                  onClick={() => setTaskIsUrgent(!taskIsUrgent)}
                ...
                >
                  <span className={taskIsUrgent ? 'animate-pulse' : ''}>🔥</span>
                  <span>{taskIsUrgent ? 'Marcada como Urgente' : 'Marcar como Urgente (Atención Inmediata)'}</span>
                </button>
              </div>
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
          ...
          {activeTab === 'meeting' && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 bg-white"
                  >
                    <option value="reunion">Reunión de Coordinación</option>
                    <option value="comite">Comité Técnico</option>
                    <option value="capacitacion">Capacitación</option>
                    <option value="consultoria">Consultoría / Asesoría</option>
                    <option value="coordinacion">Coordinación Red / Intersectorial</option>
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 bg-white"
                  >
                    <option value="programada">Programada (Por realizar)</option>
                    <option value="en_curso">En curso</option>
                    <option value="finalizada">Finalizada (Minuta lista)</option>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Título de la Instancia / Sesión *
                </label>
                <input
                  type="text"
                  required
                  placeholder="ej. Comité Técnico Comunal de Cuidados Paliativos - Sesión Extraordinaria"
                  value={meetTitle}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="ej. Sala DISAM / CESFAM Manuel Bustos"
                    value={meetLocation}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1 flex items-center justify-between">
                  <span>Enlace de Videollamada (Google Meet / Zoom / Teams)</span>
                  <span className="text-[10px] text-indigo-600 font-normal">Sincronizado con Google Calendar</span>
                </label>
                <input
                  type="url"
                  placeholder="https://meet.google.com/xyz-abcd-efg ó https://us02web.zoom.us/j/..."
                  value={meetLink}
                  onChange={(e) => setMeetLink(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 font-mono"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Descripción & Temario (Visible en Google Calendar)
                </label>
                <textarea
                  rows={2}
                  placeholder="Descripción detallada, tabla de temas a tratar, notas previas..."
              ...
              <div className="flex items-center gap-2 p-2.5 rounded-xl bg-indigo-50/80 border border-indigo-100">
                <input
                  type="checkbox"
                  id="qc-meet-sync-gcal"
                  checked={meetSyncGCal}
                  onChange={(e) => setMeetSyncGCal(e.target.checked)}
                  className="h-4 w-4 rounded border-indigo-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer"
                />
                <label htmlFor="qc-meet-sync-gcal" className="text-xs font-semibold text-indigo-950 cursor-pointer flex-1">
                  Sincronizar y agendar automáticamente en Google Calendar (con enlaces Meet/Zoom y notas)
                </label>
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="ej. Klaus Bauer, Referente Técnico, Encargada Calidad"
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Objetivos, temario y puntos clave a resolver..."
                  value={meetObjective}
              ...
              <div className="p-3.5 bg-purple-50/50 rounded-xl border border-purple-200/80 space-y-2">
                <label className="block text-xs font-bold text-purple-900 uppercase tracking-wide">
                  Acuerdos Tomados (1 por línea)
                </label>
                <textarea
                  rows={2}
                  placeholder="Escribe cada acuerdo en una línea separada..."
                  value={meetAgreementsText}
                  onChange={(e) => setMeetAgreementsText(e.target.value)}
                  className="w-full rounded-lg border border-purple-200 px-3 py-2 text-xs text-slate-800 bg-white placeholder:text-slate-400"
                />
                <p className="text-2xs text-purple-700">
                  Cada línea se registrará como un acuerdo formal de la sesión.
                </p>
              </div>
              ...
              <div className="p-3.5 bg-blue-50/50 rounded-xl border border-blue-200/80 space-y-3">
                <label className="block text-xs font-bold text-blue-900 uppercase tracking-wide">
                  Compromisos Asignados ({meetCommitmentsList.length})
                </label>

                {meetCommitmentsList.length > 0 && (
                  <div className="space-y-1.5">
                    {meetCommitmentsList.map((c, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 bg-white rounded-lg border border-blue-200 text-xs">
                        <div>
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-slate-800">{c.description}</span>
                            {c.isUrgent && (
                              <span className="text-2xs font-bold text-rose-700 bg-rose-50 px-1 py-0.2 rounded border border-rose-200">
                                Urgente
                              </span>
                            )}
                          </div>
                        ...
                        <button
                          type="button"
                          onClick={() => setMeetCommitmentsList(prev => prev.filter((_, i) => i !== idx))}
                          className="text-rose-600 hover:text-rose-800 text-xs font-bold px-1.5"
                        >
                          ✕
                        </button>
                      </div>
                ...
                <div className="space-y-2 bg-white p-3 rounded-lg border border-blue-200">
                  <input
                    ...
                    value={meetCommitmentDesc}
                    onChange={(e) => setMeetCommitmentDesc(e.target.value)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <input
                      ...
                      value={meetCommitmentResp}
                      onChange={(e) => setMeetCommitmentResp(e.target.value)}
                      className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs text-slate-800"
                    />
                    <input
                      type="date"
                      value={meetCommitmentDeadline}
                      onChange={(e) => setMeetCommitmentDeadline(e.target.value)}
                      className="w-full rounded-lg border border-slate-300 px-2.5 py-1.5 text-xs text-slate-800"
                    />
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <label className="flex items-center gap-1.5 text-xs text-slate-700 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={meetCommitmentIsUrgent}
                        onChange={(e) => setMeetCommitmentIsUrgent(e.target.checked)}
                        className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
                      />
                      <span className="font-semibold text-rose-700">Marcar como urgente</span>
                    </label>
                    <button
                      type="button"
                      onClick={() => {
                        if (!meetCommitmentDesc.trim()) return;
                        setMeetCommitmentsList(prev => [
                          ...prev,
                          {
                            description: meetCommitmentDesc.trim(),
                            responsible: meetCommitmentResp.trim() || currentUser.name,
                            deadline: meetCommitmentDeadline,
                            isUrgent: meetCommitmentIsUrgent,
                          },
                        ]);
                        setMeetCommitmentDesc('');
                        setMeetCommitmentIsUrgent(false);
                      }}
          ...
          {activeTab === 'purchase' && (
            <div className="space-y-4">
              {/* 1. DATOS GENERALES & ECONÓMICOS (PUNTO 1) */}
              <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500 mt-1">
                    {purModalidad.toLowerCase().includes('ágil') && 'Fecha en que el proveedor acepta la Orden de Compra en Mercado Público.'}
                    {purModalidad.toLowerCase().includes('convenio') && 'Fecha en que el decreto de adjudicación queda totalmente tramitado y firmado.'}
                    {purModalidad.toLowerCase().includes('licitación') && 'Fecha de cierre y firma del contrato administrativo.'}
                    {!purModalidad.toLowerCase().includes('ágil') && !purModalidad.toLowerCase().includes('convenio') && !purModalidad.toLowerCase().includes('licitación') && 'Fecha clave de formalización o aceptación del proceso de compra.'}
                  </p>
                </div>
                ...
                <div className="space-y-3 p-3 bg-white rounded-xl border border-slate-200">
                  <div>
                    ...
                    <input
                      type="date"
                      required
                      value={purCeroPapelInitiationDate}
                      onChange={(e) => setPurCeroPapelInitiationDate(e.target.value)}
                      className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800 font-medium focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-white"
                    />
                    <p className="text-[10px] text-slate-500 mt-1">
                      Fecha en que se inició formalmente el proceso en CeroPapel. Se mantiene en todos los estados (Pendiente, En ejecución, Completado).
                    </p>
                  </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                  <button
                    type="button"
                    onClick={() => {
                      setPurMacroState('pendiente');
                      setPurStatus('solicitado');
                    }}
                    className={`p-3 rounded-xl border text-left transition-all ${
                      purMacroState === 'pendiente' || purMacroState === 'por_hacer'
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    value={emailType}
                    onChange={(e) => setEmailType(e.target.value as CommunicationType)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  >
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="ej. Solicitar número de guía de despacho y fecha estimada de entrega..."
                  value={emailRequiredAction}
                  onChange={(e) => setEmailRequiredAction(e.target.value)}
                  className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Observaciones, antecedentes o instrucciones relevantes..."
                  value={emailNotes}
          ...
          {activeTab === 'question' && (
            <>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Explica el origen de la duda o el antecedente técnico/normativo..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    ¿A quién va dirigida la duda?
                  </label>
                  <input
                    type="text"
                    value={questionResponsible}
                    onChange={(e) => setQuestionResponsible(e.target.value)}
                    placeholder="ej. Dra. Marcela Vidal R. / Referente SSMN"
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setQuestionPriority(e.target.value as PriorityLevel)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  >
                    <option value="critica">Crítica</option>
                    <option value="alta">Alta</option>
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={questionFollowUpDate}
                    onChange={(e) => setQuestionFollowUpDate(e.target.value)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  />
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4 pt-1">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={questionIsUrgent}
                    onChange={(e) => setQuestionIsUrgent(e.target.checked)}
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
                <div className="sm:col-span-3">
                  ...
                  <select
                    value={indCodigo}
                    onChange={(e) => setIndCodigo(e.target.value)}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      step="any"
                      placeholder={indNumTipo === 'porcentaje' ? '0' : 'ej. 150'}
                      value={indNumPorc}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <label className="block text-xs font-semibold text-slate-700">
                      Denominador ({indDenTipo === 'porcentaje' ? '%' : 'Cantidad'})
                    </label>
                    <div className="flex items-center bg-slate-200/80 rounded-lg p-0.5 border border-slate-300">
                      <button
                        type="button"
                        onClick={() => setIndDenTipo('porcentaje')}
                        className={`px-1.5 py-0.5 text-[10px] font-bold rounded transition-all ${
                          indDenTipo === 'porcentaje'
                            ? 'bg-indigo-600 text-white shadow-xs'
                            : 'text-slate-600 hover:text-slate-900'
                        }`}
                        title="Medir en Porcentaje (%)"
                  ...
                  <div className="relative">
                    <input
                      type="number"
                      step="any"
                      placeholder={indDenTipo === 'porcentaje' ? '100' : 'ej. 200'}
                      value={indDenPorc}
                      onChange={(e) => setIndDenPorc(e.target.value)}
                      className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 pr-8 text-xs text-slate-800 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                    />
                    <span className="absolute right-2.5 top-2.5 text-xs font-bold text-slate-400">
                      {indDenTipo === 'porcentaje' ? '%' : 'N°'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {(() => {
                const currentNum = parseFloat(indCurrent) || 0;
                const pesoNum = parseFloat(indPesoRelativo) || 0;
                const metaNum = parseFloat(indMetaAnualPorc) || 100;
                const finalWeighted = ((currentNum * pesoNum) / 100).toFixed(2);
                const targetWeighted = ((metaNum * pesoNum) / 100).toFixed(2);
                const indicatorLabel = indCodigo || 'Indicador 1';
