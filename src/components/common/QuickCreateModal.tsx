...
export const QuickCreateModal: React.FC<{ isOpen: boolean; onClose: () => void; defaultProgramId?: ProgramId | null }> = ({
  ...
  const [indCode, setIndCode] = useState('IND-NUEVO-01');
  const [indName, setIndName] = useState('');
  const [indAnnualTarget, setIndAnnualTarget] = useState('100');
  const [indPeriodTarget, setIndPeriodTarget] = useState('100');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (activeTab === 'task') {
      ...
      addIndicator({
        ...
        description: indName,
        periodicity: 'Mensual',
        annualTarget: parseFloat(indAnnualTarget) || 100,
        periodTarget: parseFloat(indPeriodTarget) || 100,
        currentResult: parseFloat(indCurrent) || 0,
        unit: indUnit,
        direction: 'higher_is_better',
        cutoffDate: '2026-08-15',
        responsible: currentUser.name,
        source: 'REM / Plataforma Interna',
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm animate-in fade-in duration-150">
      <div
        ...
        className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl overflow-hidden border border-slate-200"
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 bg-slate-50">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900">Crear Nuevo Registro</h2>
              <p className="text-xs text-slate-500">Captura rápida en 1 clic — Centro Operativo Quilicura</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-wrap items-center border-b border-slate-200 bg-slate-50 px-4 py-2.5 gap-1.5">
          {tabs.map((tab) => {
            const Icon = tab.icon;
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    value={indAnnualTarget}
                    onChange={(e) => setIndAnnualTarget(e.target.value)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={indPeriodTarget}
                    onChange={(e) => setIndPeriodTarget(e.target.value)}
                    className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs text-slate-800"
                  />
                </div>
                <div>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button
              ...
              className="rounded-lg bg-indigo-600 px-5 py-2 text-xs font-semibold text-white shadow-sm hover:bg-indigo-700 active:scale-95 transition-all"
            >
              Guardar Registro
            </button>
          </div>
