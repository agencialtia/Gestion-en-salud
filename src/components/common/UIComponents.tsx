...
export const MeetingStatusChip: React.FC<{
  status?: string;
  onChange?: (newStatus: 'pendiente' | 'en_ejecucion' | 'completado') => void;
  className?: string;
}> = ({ status = 'pendiente', onChange, className = '' }) => {
  const normalize = (st?: string): 'pendiente' | 'en_ejecucion' | 'completado' => {
    if (st === 'finalizada' || st === 'cumplido' || st === 'completado' || st === 'terminada') return 'completado';
    if (st === 'en_curso' || st === 'en_progreso' || st === 'en_ejecucion') return 'en_ejecucion';
    return 'pendiente';
  };
  ...
  const configs = {
    pendiente: { bg: 'bg-slate-100', text: 'text-slate-700', border: 'border-slate-300', label: 'Pendiente' },
    en_ejecucion: { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-300', label: 'En ejecución' },
    completado: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-300 font-semibold', label: 'Completado' },
  };
  ...
  if (onChange) {
    return (
      <div className={`relative inline-flex items-center ${className}`} onClick={(e) => e.stopPropagation()}>
        <select
          id={`meeting-status-select-${current}`}
          value={current}
          onChange={(e) => onChange(e.target.value as 'pendiente' | 'en_ejecucion' | 'completado')}
          className={`appearance-none font-semibold rounded-lg border text-xs pr-6 pl-2.5 py-1 transition-all cursor-pointer shadow-2xs hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 ${c.bg} ${c.text} ${c.border}`}
...
export const CommunicationStatusChip: React.FC<{
  status?: string;
  onChange?: (newStatus: CommunicationStatus) => void;
  className?: string;
}> = ({ status, onChange, className = '' }) => {
  const normalized = normalizeCommunicationStatus(status);

  const configs: Record<CommunicationStatus, { bg: string; text: string; border: string; label: string; dot: string }> = {
    pendiente: {
      bg: 'bg-slate-100',
      text: 'text-slate-700',
  ...
  if (onChange) {
    return (
      <div className={`relative inline-flex items-center ${className}`} onClick={(e) => e.stopPropagation()}>
        <select
          id={`comm-status-select-${normalized}`}
          value={normalized}
          onChange={(e) => onChange(e.target.value as CommunicationStatus)}
          className={`appearance-none font-semibold rounded-lg border text-xs pr-6 pl-2.5 py-1 transition-all cursor-pointer shadow-2xs hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 ${c.bg} ${c.text} ${c.border}`}
