...
export const TaskStatusChip: React.FC<{ 
  ...
  const configs: Record<TaskOperationalCategory, { bg: string; text: string; border: string; label: string }> = {
    ...
    en_ejecucion: { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-300', label: 'En gestión' },
    ...
    return (
      <div className={`relative inline-flex items-center ${className}`} onClick={(e) => e.stopPropagation()}>
        <select
          ...
          title="Cambiar estado: Pendiente, En gestión, Cerrado, Vence hoy, Vencido"
        >
          <option value="pendiente">Pendiente</option>
          <option value="en_ejecucion">En gestión</option>
          <option value="completado">Cerrado / Completado</option>
...
export type MeetingOperationalCategory = 'pendiente' | 'en_ejecucion' | 'completado' | 'vence_hoy' | 'vencido';
...
export const MeetingStatusChip: React.FC<{
  ...
  onChange?: (newStatus: any) => void;
  className?: string;
}> = ({ status = 'pendiente', dateTime, hasOverdueCommitments, category, onChange, className = '' }) => {
  const todayStr = '2026-08-15';
  ...
  const currentCategory: MeetingOperationalCategory = (() => {
    ...
    if (isDone) return 'completado';
    if (hasOverdueCommitments) return 'vencido';
    if (dateTime) {
      const datePart = dateTime.split('T')[0];
      if (datePart < todayStr) return 'vencido';
      if (datePart === todayStr) return 'vence_hoy';
    }
