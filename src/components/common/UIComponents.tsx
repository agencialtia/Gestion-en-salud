...
import { TrafficLightStatus, PriorityLevel, TaskStatus, PurchaseStatus, AlertSeverity } from '../../types';
import { 
  CheckCircle2, 
  AlertTriangle, 
  AlertCircle, 
...
export const PurchaseStatusChip: React.FC<{ status: PurchaseStatus }> = ({ status }) => {
  const configs = {
    pendiente: { bg: 'bg-slate-100 text-slate-700', label: 'Pendiente' },
    solicitado: { bg: 'bg-amber-100 text-amber-800', label: 'Solicitado' },
    en_compra: { bg: 'bg-indigo-100 text-indigo-800', label: 'En Compra' },
    recepcionado: { bg: 'bg-emerald-100 text-emerald-800', label: 'Recepcionado' },
    cerrado: { bg: 'bg-slate-200 text-slate-800', label: 'Cerrado' },
    problema: { bg: 'bg-rose-100 text-rose-800 font-semibold', label: 'Problema / Traba' },
  };
  const c = configs[status] || configs.pendiente;
  return (
    <span id={`purchase-status-${status}`} className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${c.bg}`}>
      {c.label}
    </span>
  );
};
