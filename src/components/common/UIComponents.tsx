...
export const ProgramBadge: React.FC<{ programId: string; shortName?: string; color?: string; size?: 'sm' | 'md' | 'lg' }> = ({
  programId,
  shortName,
  color = '#0284c7',
  size = 'md',
  ...
  const paddingClass = size === 'sm' ? 'px-1.5 py-0.5 text-[11px]' : size === 'lg' ? 'px-2.5 py-1 text-sm' : 'px-2 py-0.5 text-xs';

  return (
    <span
      id={`program-badge-${programId}`}
      className={`inline-flex items-center gap-1.5 rounded-md font-medium bg-slate-100 text-slate-800 border border-slate-200 ${paddingClass}`}
    >
...
export const ProgressBar: React.FC<{
  ...
  colorClass?: string;
  size?: 'sm' | 'md' | 'lg';
}> = ({ value, percent, max = 100, showText = false, colorScheme = 'auto', colorClass, size = 'md' }) => {
  const actualVal = percent !== undefined ? percent : (value || 0);
  const percentage = Math.min(Math.max(0, (actualVal / max) * 100), 100);
...
export const QuestionCategoryBadge: React.FC<{
  category: QuestionCategory | string;
  className?: string;
}> = ({ category, className = '' }) => {
  const configs: Record<string, { bg: string; text: string; border: string; label: string }> = {
    tecnica: { bg: 'bg-cyan-50', text: 'text-cyan-800', border: 'border-cyan-200', label: 'Técnica / Clínica' },
    normativa: { bg: 'bg-purple-50', text: 'text-purple-800', border: 'border-purple-200', label: 'Normativa / Legal' },
    administrativa: { bg: 'bg-slate-100', text: 'text-slate-700', border: 'border-slate-300', label: 'Administrativa' },
    financiera: { bg: 'bg-emerald-50', text: 'text-emerald-800', border: 'border-emerald-200', label: 'Financiera' },
    servicio_salud: { bg: 'bg-indigo-50', text: 'text-indigo-800', border: 'border-indigo-200', label: 'SSMN' },
    gestion_interna: { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-200', label: 'Gestión Interna' },
    otra: { bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-slate-200', label: 'Otra' },
  };

  const c = configs[category] || configs.otra;

  return (
    <span
      id={`question-cat-badge-${category}`}
      className={`inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-bold uppercase tracking-wider border ${c.bg} ${c.text} ${c.border} ${className}`}
    >
      {c.label}
    </span>
  );
...
export const QuestionStatusChip: React.FC<{
  status: QuestionStatus;
  followUpDate?: string;
  onChange?: (newStatus: QuestionStatus) => void;
  className?: string;
}> = ({ status, followUpDate, onChange, className = '' }) => {
  const normalizedStatus = status === 'abierta' ? 'pendiente' : status === 'cerrada_sin_respuesta' ? 'resuelta' : status;

  const configs: Record<string, { bg: string; text: string; border: string; label: string }> = {
    pendiente: { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-300 font-semibold', label: 'Pendiente' },
    abierta: { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-300 font-semibold', label: 'Pendiente' },
    en_consulta: { bg: 'bg-blue-50', text: 'text-blue-700', border: 'border-blue-300 font-semibold', label: 'En consulta' },
    esperando_respuesta: { bg: 'bg-amber-50', text: 'text-amber-800', border: 'border-amber-300 font-semibold', label: 'Esperando respuesta' },
    resuelta: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-300 font-bold', label: 'Resuelta' },
    cerrada_sin_respuesta: { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-300 font-bold', label: 'Resuelta' },
  };

  const c = configs[normalizedStatus] || configs.pendiente;

  if (onChange) {
    return (
      <div className={`relative inline-flex items-center ${className}`} onClick={(e) => e.stopPropagation()}>
        <select
          id={`question-status-select-${normalizedStatus}`}
          value={normalizedStatus}
          onChange={(e) => onChange(e.target.value as QuestionStatus)}
          className={`appearance-none font-semibold rounded-lg border text-xs pr-6 pl-2.5 py-1 transition-all cursor-pointer shadow-2xs hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 ${c.bg} ${c.text} ${c.border}`}
          title="Cambiar estado: Pendiente, En consulta, Esperando respuesta, Resuelta"
        >
          <option value="pendiente">Pendiente</option>
  ...
  return (
    <span
      id={`question-status-${normalizedStatus}`}
      className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-semibold border ${c.bg} ${c.text} ${c.border} ${className}`}
    >
      {c.label}
    </span>
...
export const QuestionOverdueBadge: React.FC<{
  followUpDate?: string;
  status?: QuestionStatus;
  currentDate?: string;
  className?: string;
}> = ({ followUpDate, status, currentDate = '2026-08-15', className = '' }) => {
  if (!followUpDate || status === 'resuelta' || status === 'cerrada_sin_respuesta') {
    return null;
  }

  const isOverdue = followUpDate < currentDate;
  const isToday = followUpDate === currentDate;

  if (isOverdue) {
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold bg-rose-100 text-rose-800 border border-rose-300 animate-pulse ${className}`}
        title={`Seguimiento vencido (${formatDate(followUpDate)})`}
      >
        <AlertCircle className="w-3 h-3 text-rose-600 shrink-0" />
  ...
  if (isToday) {
    return (
      <span
        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold bg-amber-100 text-amber-900 border border-amber-300 ${className}`}
        title={`Seguimiento hoy (${formatDate(followUpDate)})`}
      >
        <Clock className="w-3 h-3 text-amber-700 shrink-0" />
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-200 ${className}`}
      title={`Seguimiento: ${formatDate(followUpDate)}`}
    >
      <Calendar className="w-3 h-3 text-slate-400 shrink-0" />
      <span>Seguimiento: {formatDate(followUpDate)}</span>
    </span>
  );
};
