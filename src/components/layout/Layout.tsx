      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden transition-all duration-300">
        ...
        <main className="flex-1 overflow-y-auto p-3 sm:p-6 lg:p-8">
          <div className="mx-auto max-w-7xl">
            <ErrorBoundary key={activeView + (selectedProgramId || '')}>
              {(activeView === 'dashboard' || activeView === 'dashboard_global' || activeView === 'hoy') && (
                <DashboardGlobalView
              ...
              {activeView === 'contactos' && (
                <ContactosView onOpenEntity={handleOpenEntity} />
              )}
              {activeView === 'documentos' && (
                <DocumentosView onOpenEntity={handleOpenEntity} />
              )}
              {activeView === 'alertas' && (
          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl border border-slate-200 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-slate-100">
              <button
                ...
                className="flex-1 rounded-xl bg-indigo-600 px-3 py-2 text-xs font-bold text-white hover:bg-indigo-700 active:scale-95 transition-all text-center"
              >
                Respondido
              </button>
              <button
